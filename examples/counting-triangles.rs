...
fn main() {
    ...
    // Create a new Matrix instance from the adjacency matrix
    let matrix = Matrix::from(adjacency_matrix);

    // Print the original matrix
    println!("Original Matrix: {}", matrix);

    // Perform element-wise multiplication with itself
    let squared_matrix = matrix.mul(matrix);

    // Print the squared matrix
    println!("\nSquared Matrix: {}", squared_matrix);
}
...
struct CountTringlesProver<const N: usize> {
    a: Matrix<N, N>,
}

impl<const N: usize> CountTringlesProver<N> {
    fn new(a: Matrix<N, N>) -> Self {
        Self { a }
    }

    fn f_a(&self, i: usize) -> BaseField {
        assert!(i < N * N);
        todo!()
    }
}

struct CountTringlesVerifier;

impl CountTringlesVerifier {
    fn verify<const N: usize>(a: Matrix<N, N>, prover: &CountTringlesProver<N>) {
        assert!(N.is_power_of_two());
    }
}

#[derive(Debug, Clone, Copy)]
struct Matrix<const M: usize, const N: usize>([[BaseField; N]; M]);

impl<const M: usize, const N: usize> Matrix<M, N> {
    fn new(data: [[BaseField; N]; M]) -> Self {
        Matrix(data)
    }
}

impl<const M: usize, const N: usize, T> From<[[T; N]; M]> for Matrix<M, N>
where
    BaseField: From<T>,
{
    fn from(value: [[T; N]; M]) -> Self {
        Matrix(value.map(|row| row.map(BaseField::from)))
    }
}
...
impl<const M: usize, const N: usize> Mul<Matrix<N, M>> for Matrix<M, N> {
    ...
    fn mul(self, other: Matrix<N, M>) -> Self {
        let mut result = [[BaseField::zero(); N]; M];
