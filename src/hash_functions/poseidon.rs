...
impl PoseidonHasher {
    pub fn default(initial_state: PoseidonHash) -> Self {
        Self {
    ...
    pub fn get_state_output(&self) -> &[BaseField; POSEIDON_CAPACITY] {
        self.state.0[0..POSEIDON_CAPACITY].try_into().unwrap()
    }
...
impl Hasher for PoseidonHasher {
    ...
    fn new() -> Self {
        Self::default(PoseidonHash([BaseField::zero(); POSEIDON_WIDTH]))
    }

    fn reset(&mut self) {
        self.state = PoseidonHash([BaseField::zero(); POSEIDON_WIDTH])
    }

    fn update(&mut self, data: &[BaseField]) {
        let mut new_state = self.state.0;
    ...
    fn hash_one_in_place(data: &[Self::NativeType], dst: &mut [Self::NativeType]) {
        ...
        dst.copy_from_slice(hasher.get_state_output());
    }
...
mod tests {
    ...
    use crate::hash_functions::poseidon::{PoseidonHash, POSEIDON_CAPACITY, POSEIDON_WIDTH};
    use crate::m31;
    ...
    fn hades_permutation_regression_test() {
        ...
        let expected_state = PoseidonHash(ZERO_HASH_RESULT);

        assert_eq!(hasher.finalize(), expected_state);
    }

    #[test]
    fn finalize_reset_test() {
        let initial_state = PoseidonHash([m31!(0); POSEIDON_WIDTH]);
        let mut hasher = PoseidonHasher::default(initial_state);

        let res = hasher.finalize_reset();

        assert_ne!(res, hasher.state);
        assert_eq!(hasher.state, initial_state);
    }

    #[test]
    fn hash_one_in_place_test() {
        let input = vec![m31!(0); POSEIDON_WIDTH];
        let mut output = vec![m31!(0); POSEIDON_CAPACITY];

        PoseidonHasher::hash_one_in_place(&input, &mut output);

        assert_eq!(output, ZERO_HASH_RESULT[0..POSEIDON_CAPACITY]);
    }
}
