...
impl<T: Sized + Copy + Default + Debug + Display, H: Hasher> MerkleTree<T, H> {
    ...
    pub fn generate_decommitment<const LEAF_SIZE: usize>(
        ...
        leaf_block_indices.iter().for_each(|index| {
            proof.leaves.push(self.get_leaf_block::<LEAF_SIZE>(*index));
        });
        ...
        for i in 0..self.height - 2 {
            ...
            for j in curr_layer_indices.iter() {
                // If an index is a parent it can be calculated from it's children and does not need
                // to be included in the proof.
                if !curr_layer_indices.contains(&(*j ^ 1)) {
                    let block = &self.data[i]
                        [*j * H::OUTPUT_SIZE_IN_BYTES..(*j + 1) * H::OUTPUT_SIZE_IN_BYTES];
                    proof_layer.push(block.try_into().expect("Error convering slice to array"));
                }
            }
