...
use crate::commitment_scheme::utils::{
    allocate_balanced_tree, hash_merkle_tree_from_bottom_layer, inject, to_byte_slice,
...
impl<T: Sized, H: Hasher> MerkleTree<T, H> {
    ...
    fn init_from_column_array(mut trace: ColumnArray<T>) -> Self {
        ...
        // Builds bottom layer. Performes a transpose on a matrix, and a move on a column.
        let bottom_layer = if trace.len() > 1 {
            // Allocate bottom layer.
            let mut bottom_layer: Vec<T> = Vec::with_capacity(trace.len() * trace[0].len());

            // Inject(transpose).
            // Safe because enough memory is allocated.
            unsafe {
                let bottom_layer_byte_slice = std::slice::from_raw_parts_mut(
                    bottom_layer.as_mut_ptr() as *mut u8,
                    trace.len() * trace[0].len() * std::mem::size_of::<T>(),
                );
                inject(&trace, bottom_layer_byte_slice, 1, 0);
                bottom_layer.set_len(trace[0].len() * trace.len());
            }
            bottom_layer
        } else {
            // Move.
