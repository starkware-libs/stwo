...
impl<T: Hasher> MerkleTree<T> {
    ...
    pub fn from_leaves(leaves: Vec<u32>) -> Self{
        ...
        Self { data: (Vec::with_capacity(tree_height)), leaves , height: (tree_height) }
    }
    ...
    pub fn root_hex(&mut self) -> String {
        format!("{}",self.data.last().expect("Attempted access to uncomitted tree")[0])
    }
    ...
    pub fn root_without_caching(leaves: &[u32]) -> T::Hash {
        // Hash first layer 
        let mut curr_layer: Vec<T::Hash> = leaves.iter().map(|n| T::hash(&n.to_be_bytes())).collect();
