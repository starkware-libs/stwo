...
impl<T: Sized + Copy + Default + Debug + Display, H: Hasher> MerkleTree<T, H> {
    ...
    pub fn generate_decommitment<const LEAF_SIZE: usize>(
        ...
        // Input layer. Every leaf is the size of a Hash block.
                ...
                match indices_iterator.peek() {
                    Some(next_q) => {
                        if !(*q == *next_q ^ 1 && q % 2 == 0) {
                            ...
                            proof_layer
                                .push(block.try_into().expect("Error converting slice to array"));
                        } else {
    ...
    // TODO(Ohad): remove generic argument 'LEAF SIZE' and replace logic with consts from hasher.
    fn get_leaf_block<const LEAF_SIZE: usize>(&self, block_index: usize) -> [T; LEAF_SIZE] {
