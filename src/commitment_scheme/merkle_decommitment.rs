...
#[derive(Default)]
pub struct MerkleDecommitment<T: Sized + Display, H: ComplexHasher> {
    pub leaf_blocks: Vec<Vec<T>>,
    pub layers: Vec<Vec<H::Hash>>,
    pub n_rows_in_leaf_block: usize,
}

impl<T: Sized + Display, H: ComplexHasher> MerkleDecommitment<T, H>
where
...
impl<T: Sized + Display, H: ComplexHasher> fmt::Display for MerkleDecommitment<T, H> {
