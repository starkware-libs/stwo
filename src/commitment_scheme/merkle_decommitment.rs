    ...
    T: IntoSlice<H::NativeType>,
{
    ...
    pub fn values(&self) -> impl Iterator<Item = Vec<T>> + '_ {
        QueriedValuesIterator {
            ...
            leaf_block_iterator: self.leaf_blocks.iter(),
...
pub struct QueriedValuesIterator<'a, T: Sized + Display> {
    ...
    leaf_block_iterator: std::slice::Iter<'a, Vec<T>>,
        ...
        match self.query_iterator.next() {
            Some(query) => {
                ...
                if leaf_block_index != self.current_leaf_block_index {
                    self.leaf_block_iterator.advance_by(1).expect("msg");
                ...
                Some(
                    self.leaf_block_iterator.clone().next().unwrap().to_vec()
...
mod tests {
    ...
    fn get_values_at_test() {
        ...
        let trace: ColumnArray<M31> = vec![(0..trace_column_length)
            .map(M31::from_u32_unchecked)
            .collect()];
