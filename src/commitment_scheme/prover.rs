...
use super::utils::{opt_flatten_peekable, peek_merged_queries};
    ...
    pub fn decommit(
        ...
        for layer_log_size in (0..self.layers.len() as u32).rev() {
            ...
            let mut layer_column_queries =
                opt_flatten_peekable(queries_per_log_size.get(&layer_log_size));

            // Merge previous layer queries and column queries.
            while let Some(node_index) =
                peek_merged_queries(&mut prev_layer_queries, &mut layer_column_queries)
            {
