...
#[allow(dead_code)]
fn get_queried_node_indices_in_layer(
    queries: impl Iterator<Item = Vec<usize>>,
    input: &MerkleTreeInput<'_, impl Field>,
    layer_depth: usize,
) -> Vec<usize> {
    let column_lengths = input.get_columns(layer_depth).iter().map(|c| c.len());
    let mut node_queries = queries
        .into_iter()
        .zip(column_lengths)
        .flat_map(|(column_queries, column_length)| {
            let n_bags_in_layer = 1 << (layer_depth - 1);
    ...
    fn translate_queries<F: Field>(
        ...
        (1..=input.max_injected_depth())
            ...
            .map(|i| {
                ...
                super::get_queried_node_indices_in_layer(column_queries_at_depth, input, i)
            })
