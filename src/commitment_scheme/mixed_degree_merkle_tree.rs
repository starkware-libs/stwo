    ...
    F: IntoSlice<H::NativeType>,
{
    ...
    // Ancestor - a node index that is part of the witness for a query below (ancestor of a query).
    // queries to layer - every query to a specific index at a specific column that is injected at
    // the current depth. queried_node_indices - the node (bag) indices that contain the above
    // queries. ancestors_of_previous_layer - all ancestors of previous layers.
    #[allow(dead_code)]
    fn decommit_single_layer(
        &self,
        layer_depth: usize,
        queries_to_layer: impl Iterator<Item = &'a Vec<usize>> + Clone,
        mut ancestors_of_previous_layers_indices: Peekable<impl Iterator<Item = usize> + Clone>,
        ...
        let mut index_value_iterator = directly_queried_node_indices
            ...
            .peekable();
        let node_indices = MergeIter::new(
            directly_queried_node_indices.iter().copied(),
            ancestors_of_previous_layers_indices.clone().map(|q| q / 2),
        )
        .collect_vec();

        for node_index in node_indices.iter() {
            if layer_depth < self.input.max_injected_depth() {
                match ancestors_of_previous_layers_indices.next_if(|&q| q / 2 == *node_index) {
                    Some(q) => {
                        match ancestors_of_previous_layers_indices
                            .next_if(|&next_q| q == next_q ^ 1)
                        {
                            Some(_) => {
                                break;
                            }
            ...
            match index_value_iterator.next_if(|(n, _)| *n == *node_index) {
                Some((_, (witness, queried))) => {
