    ...
    F: IntoSlice<H::NativeType>,
{
    ...
    pub fn get_hash_at(&self, layer_depth: usize, position: usize) -> H::Hash {
        ...
        let mut multi_layer_top_to_bottom_iterator = self.multi_layers.iter().rev();
        let mut depth_accumulator = layer_depth;
        let mut current_multi_layer = multi_layer_top_to_bottom_iterator.next().unwrap();
        loop {
            let current_multi_layer_height = current_multi_layer.config.sub_tree_height;
            if current_multi_layer_height > depth_accumulator {
                break;
            }
            depth_accumulator -= current_multi_layer.config.sub_tree_height;
            current_multi_layer = multi_layer_top_to_bottom_iterator
                .next()
                .expect("Invalid layer depth");
        }

        current_multi_layer.get_hash_value(depth_accumulator, position)
    }
