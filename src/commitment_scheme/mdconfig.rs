...
/// A map from the depth of the tree requested to be inject to the to-be-injected columns.
/// 
/// # Example
/// 
/// ```
...
impl <'a, F: Field> MDConfig<'a, F> {
    pub fn insert(&mut self, depth: usize, column: &'a Vec<F>) -> Result<(),MDConfigError> {
        ...
        if column.len() < 2usize.pow(depth as u32) {
            return Err(MDConfigError::DepthTooLarge)
        }
    ...
    pub fn keys(&self) -> std::collections::btree_map::Keys<'_, usize, Vec<&'a Vec<F>>> {
        self.map.keys()
    }

    pub fn split_off(&mut self, key: &usize) -> MDConfig<'a, F> {
        let new_config = Self{map : self.map.split_off(key), max_depth: self.max_depth };
        self.max_depth = *self.map.keys().last().unwrap_or(&0);
        new_config
    }
}
...
mod tests {
    ...
    pub fn md_config_extract_test() {
        ...
        assert_eq!(column_too_small_result, super::MDConfigError::DepthTooLarge);
        assert_eq!(column_not_power_of_two_result, super::MDConfigError::ColumnSizeNotPowerOfTwo);
    }
}