...
impl super::hasher::Hasher for Blake3Hasher {
    ...
    fn hash_one_xof(data: &[u8], dst: &mut [u8]) {
        assert!(
            dst.len() >= Self::OUTPUT_SIZE_IN_BYTES,
            "Attempt to Generate blake3 hash of size different smaller than 32 bytes!"
        );
        let mut hasher = blake3::Hasher::new();
        hasher.update(data);
    ...
    unsafe fn hash_many_xof(data: &[*const u8], single_input_length_bytes: usize, dst: &[*mut u8]) {
        data.iter()
            ...
            .for_each(|(input, out)| Self::hash_one_xof(input, out))
    }
}
...
mod tests {
    ...
    fn hash_xof_test() {
        ...
        Blake3Hasher::hash_one_xof(input, &mut out[..]);
        assert_eq!(
            "17762fddd969a453925d65717ac3eea21320b66b54342fde15128d6caf21215f",
            hex::encode(out)
        )
    ...
    fn hash_many_xof_test() {
        ...
        unsafe { Blake3Hasher::hash_many_xof(&input_arr, 1, &out_ptrs) };
