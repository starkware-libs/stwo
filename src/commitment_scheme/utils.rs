    ...
    F: IntoSlice<H::NativeType>,
{
    for column in columns.iter() {
        let column_slice = view_column_chunk(column, chunk_idx, n_chunks_in_column);
        ...
        let n_rows_in_chunk = column_slice.len() / hash_inputs.len();
        column_slice
...
#[inline]
fn view_column_chunk<F>(column: &[F], index_to_view: usize, n_total_chunks: usize) -> &[F] {
...
mod tests {
    ...
    fn inject_columns_test() {
        ...
        inject_columns::<Blake3Hasher, M31>(&columns, &mut hash_inputs, 1, 2);
