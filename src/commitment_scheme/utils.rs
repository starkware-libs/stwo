...
pub unsafe fn inject<T: Sized>(
    ...
    let ptr_offset = column_array.len() * n_rows_in_node * std::mem::size_of::<T>() + offset;
    let offseted_pointers: Vec<*mut u8> = (offset..dst.len())
        .step_by(ptr_offset)
...
mod tests {
    ...
    fn inject_test() {
        ...
        let mut out = [0_u8; 36];
        unsafe {
            inject::<u32>(&columns_to_transpose, &mut out[..], 1, offset);
        }
        // offset[1,5]offset[2,6]offset...
        assert_eq!(
            hex::encode(&out[..14]),
