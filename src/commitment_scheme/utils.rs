    ...
    F: IntoSlice<H::NativeType>,
{
    ...
    if IS_INTERMEDIATE {
        assert_eq!(
            ...
            "prev hashes len: {} but dst length: {}",
            prev_hashes.len(),
        ...
        assert_eq!(
            prev_hashes.len(),
            0,
            "Child values for non-intermediate layer!"
        );
    }
    ...
    match input_columns.clone().peekable().next() {
        Some(_) => {
            ...
            dst.iter_mut().enumerate().for_each(|(i, dst)| {
                ...
                if IS_INTERMEDIATE {
                    inject_previous_hash_values::<H>(i, &mut hasher, prev_hashes);
                }
...
mod tests {
    ...
    fn inject_and_hash_layer_test() {
        ...
        let mut dst_layer_2 = vec![<commitment_scheme::blake3_hash::Blake3Hasher as commitment_scheme::hasher::Hasher>::Hash::default();1 << (sub_trees_height-1)];
        ...
        // dst_layer_0: [Blake3Hash;8] = [h(0,1),h(2,3),...h(14,15)]
        dst_layer_0.iter().enumerate().for_each(|(i, h)| {
        ...
        dst_layer_2.iter().enumerate().for_each(|(i, h)| {
            let mut hasher = Blake3Hasher::new();
            hasher.update(dst_layer_0[i * 2].as_ref());
            hasher.update(dst_layer_0[i * 2 + 1].as_ref());
            assert_eq!(h, &hasher.finalize());
    ...
    #[test]
    #[should_panic]
    fn inject_and_hash_layer_empty_test() {
        let layer_length = 1 << 3;
        let prev_hashes = vec![<commitment_scheme::blake3_hash::Blake3Hasher as commitment_scheme::hasher::Hasher>::Hash::default();layer_length - 1];
