...
mod tests {
    ...
    pub fn md_config_insert_test() {
        let mut config = super::MerkleTreeInput::<M31>::new();
        let column = vec![M31::from_u32_unchecked(0); 1024];

        config.insert_column(3, &column);
        config.insert_column(3, &column);
        config.insert_column(2, &column);
    ...
    pub fn mt_input_column_too_short_test() {
        ...
        config.insert_column(11, &column);
    }
    ...
    pub fn mt_input_wrong_size_test() {
        ...
        config.insert_column(2, &not_pow_2_column);
    }
