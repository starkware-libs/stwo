...
impl super::hasher::Hasher for Blake2sHasher {
    ...
    // TODO(Ohad): implement a Hash trait that requires 'Sized', then do a box allocation here, and
    // remove #[allow(clippy::uninit_vec))]
    #[allow(clippy::uninit_vec)]
    fn hash_many_multi_src(data: &[Vec<&[u8]>]) -> Vec<Self::Hash> {
        let mut out = Vec::with_capacity(data.len());

        // Safe because no reads are attempted before writing the entire vector.
        unsafe { out.set_len(data.len()) };
        Self::hash_many_multi_src_in_place(data, &mut out);
        out
    }
