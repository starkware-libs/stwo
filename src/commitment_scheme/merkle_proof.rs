...
impl<const NODE_SIZE: usize> MerklePath<NODE_SIZE> {
    ...
    pub fn set_root(&mut self, val: [u8; NODE_SIZE]) {
        self.root = Some(val);
    }

    pub fn root(&self) -> Result<[u8; NODE_SIZE], NoRootError> {
        match self.root {
            Some(root) => Ok(root),
            None => Err(NoRootError),
        }
    }
...
impl fmt::Display for NoRootError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "No root set for MerklePath!")
    }
}
...
mod tests {
    ...
    #[test]
    pub fn no_root_test() {
        let m_path = MerklePath::<32>::new(3);
        assert!(m_path.root().is_err());
    }
