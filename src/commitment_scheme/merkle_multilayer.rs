        ...
        H::Hash: 'a,
    {
        ...
        for i in (1..=self.config.sub_tree_height).rev() {
            ...
            while let Some(q) = curr_layer_indices.next() {
                let mut get_required_hash = || -> Option<_> {
                    match curr_layer_indices.peek() {
                ...
                let hash_value = if i < self.config.sub_tree_height {
                    get_required_hash()
                } else if IS_INTERMEDIATE {
                    // Get value from previous layer.
                    let prev_hash = previous_hashes.clone().nth(*q).unwrap();
                    Some(*prev_hash)
                } else {
                    None
                };
                let injected_elements = self.get_injected_elements(input, i, *q);
                if hash_value.is_some() || !injected_elements.is_empty() {
                    let position_in_layer = if IS_INTERMEDIATE || i != self.config.sub_tree_height {
                        PositionInLayer::new_child(*q)
                    } else {
                        PositionInLayer::Leaf(*q / 2)
                    };
