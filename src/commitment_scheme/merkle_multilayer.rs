...
impl<H: Hasher> MerkleMultiLayer<H> {
    ...
    pub fn generate_decommitment<'a, F: Field, const IS_INTERMEDIATE: bool>(
        &self,
        input: &MerkleTreeInput<'_, F>,
        relative_depth: usize,
        mut queried_indices: BTreeSet<usize>,
        previous_hashes: impl ExactSizeIterator<Item = &'a H::Hash> + Clone,
        ...
        H::Hash: 'a,
    {
        if IS_INTERMEDIATE {
            assert_eq!(
                previous_hashes.len(),
                self.config.n_sub_trees * (1 << self.config.sub_tree_height),
                "Provided previous hashes are not of the correct length."
            );
        }
        ...
        let mut leaf_layer_indices = queried_indices.iter().peekable();
        while let Some(q) = leaf_layer_indices.next() {
            let hash = match leaf_layer_indices.peek() {
                // If both children are in the layer, only injected elements are needed
                // to calculate the parent.
                Some(next_q) if q ^ 1 == **next_q => {
                    leaf_layer_indices.next();
                    None
                }
                _ => Some(previous_hashes.clone().nth(*q).unwrap().to_owned()),
            };
            let position_in_layer = if IS_INTERMEDIATE {
                PositionInLayer::new_child(*q)
            } else {
                PositionInLayer::Leaf(*q)
            };
            let injected_elements =
                self.get_injected_elements(input, self.config.sub_tree_height + relative_depth, *q);
        ...
        for i in (1..self.config.sub_tree_height).rev() {
            ...
            let mut curr_layer_indices = queried_indices.iter().peekable();
            while let Some(q) = curr_layer_indices.next() {
                let hash_witness = match curr_layer_indices.peek() {
                    // If both children are in the layer, only injected elements are needed
                    // to calculate the parent.
                    Some(next_q) if q ^ 1 == **next_q => {
                        curr_layer_indices.next();
                        None
                    }
                    _ => Some(self.get_hash_value(i, *q)),
                };
                let injected_elements = self.get_injected_elements(input, i + relative_depth, *q);
                if hash_witness.is_some() || !injected_elements.is_empty() {
                    let position_in_layer = PositionInLayer::new_child(*q);
    ...
    fn get_hash_value(&self, layer: usize, node_idx: usize) -> H::Hash {
        let tree_idx = node_idx / (1 << (self.config.sub_tree_height - 1));
        let sub_tree_data = self
            .data
            .chunks(self.config.sub_tree_size)
            .nth(tree_idx)
            .unwrap();
        let layer_view = &sub_tree_data[sub_tree_data.len() + 1 - (2 << layer)..];
        layer_view[node_idx % (1 << layer)]
    }
...
mod tests {
    ...
    fn decommit_layer_test() {
        ...
        let decommitment = multi_layer.generate_decommitment::<M31, true>(
            &input,
            0,
            [0, 2, 14, 30].into(),
            prev_hashes.iter(),
        ...
        assert_eq!(
            decommitment.decommitment_layers[0][3].position_in_layer,
            PositionInLayer::Left(30)
        );
        ...
        assert_eq!(
            decommitment.decommitment_layers[2][1].position_in_layer,
            PositionInLayer::Left(2)
        );
        assert_eq!(
            decommitment.decommitment_layers[2][2].position_in_layer,
            PositionInLayer::Left(6)
        );

        // Layer #3
        assert_eq!(
            decommitment.decommitment_layers[3][0].position_in_layer,
            PositionInLayer::Left(2)
        );
    }
}
