...
impl<H: Hasher> MerkleMultiLayer<H> {
        ...
        H::Hash: 'a,
    {
        ...
        // Leaf layer.
        if !IS_INTERMEDIATE {
            proof_layers.push(self.decommit_leaf_layer(queried_indices.iter().peekable(), input));
        }
    ...
    fn decommit_leaf_layer<'a, F: Field>(
        &self,
        leaf_layer_indices: Peekable<impl Iterator<Item = &'a usize>>,
        input: &MerkleTreeInput<'_, F>,
    ...
    fn decommit_intermediate_layer<'a, F: Field>(
        &self,
        layer_depth: usize,
        mut layer_indices: Peekable<impl Iterator<Item = &'a usize>>,
        input: &MerkleTreeInput<'_, F>,
    ) -> Vec<DecommitmentNode<F, H>> {
        let mut proof_layer = Vec::<DecommitmentNode<F, H>>::new();
        while let Some(q) = layer_indices.next() {
            let sibling_index = *q ^ 1;
            let hash_witness = match layer_indices.peek() {
