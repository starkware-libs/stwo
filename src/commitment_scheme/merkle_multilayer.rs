...
impl<H: Hasher> MerkleMultiLayer<H> {
    ...
    pub fn generate_decommitment<F: Field>(
        ...
        queried_indices.iter().for_each(|q| {
            ...
            for column in input.get_columns(self.config.sub_tree_height).iter() {
                let chunk = get_column_chunk(column, tree_idx, self.config.n_sub_trees)
                    .chunks(column.len() / (1 << (self.config.sub_tree_height)))
                    .nth(q % (1 << (self.config.sub_tree_height - 1)))
                    .unwrap();
        ...
        // Rest of the layers.
        queried_indices = queried_indices.into_iter().map(|index| index ^ 1).collect();
        for i in (1..self.config.sub_tree_height).rev() {
            let mut proof_layer =
                Vec::<DecommitmentNode<F, H>>::with_capacity(queried_indices.len());
            let mut indices_iterator = queried_indices.iter().peekable();
            while let Some(q) = indices_iterator.next() {
                ...
                let hash_value = f();
                let mut injected_elements = Vec::<F>::new();
                for column in input.get_columns(i).iter() {
                    let injected_chunk =
                        get_column_chunk(column, tree_idx, self.config.n_sub_trees)
                            .chunks(column.len() / (1 << i))
                            .nth((q / 2) % (1 << (i - 1)))
                            .unwrap();
                    injected_elements.extend(injected_chunk);
                }
...
mod tests {
    ...
    fn decommit_layer_non_intermediate_test() {
        ...
        // Siblings are mapped to the same parent node
        ...
        assert_eq!(
            decommitment.decommitment_layers[1][0].position_in_layer, 0
        );
        ...
        assert_eq!(
            ...
            decommitment.decommitment_layers[1][1].position_in_layer, 6
        );
        ...
        assert_eq!(
            decommitment.decommitment_layers[1][2].position_in_layer, 14
        );
