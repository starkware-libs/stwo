...
impl<B: Backend> CommitmentSchemeProver<B> {
    pub fn new(log_blowup_factor: u32) -> Self {
        CommitmentSchemeProver {
            trees: TreeVec::<CommitmentTreeProver<_>>::new(),
            log_blowup_factor,
        }
    }
    ...
    pub fn polys(&self) -> TreeColumns<&CirclePoly<B>> {
        self.trees.to_cols(|tree| tree.polynomials.iter().collect())
    }

    fn evaluations(&self) -> TreeColumns<&CircleEvaluation<B, BaseField, BitReversedOrder>> {
