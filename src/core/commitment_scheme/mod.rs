...
pub struct Decommitments(TreeVec<MerkleDecommitment<BaseField, MerkleHasher>>);

impl Deref for Decommitments {
    type Target = TreeVec<MerkleDecommitment<BaseField, MerkleHasher>>;
...
impl CommitmentSchemeProver {
    ...
    pub fn open_values(
        ...
        let decommitments = Decommitments(decommitment_results.map(|(_, d)| d));
...
pub struct CommitmentTreeProver {
    ...
    pub commitment: MerkleTree<BaseField, MerkleHasher>,
}
...
impl CommitmentTreeVerifier {
    pub fn new(commitment: Blake2sHash, log_sizes: Vec<u32>, channel: &mut Blake2sChannel) -> Self {
