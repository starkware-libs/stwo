...
impl<F: ExtensionOf<BaseField>> CircleEvaluation<F> {
    ...
    pub fn fetch_eval_on_coset(&self, coset: Coset) -> CyclicSlice<'_, F> {
        ...
        if let Some(d) = self.domain.half_coset.find(coset.initial_index) {
            return CyclicSlice {
                evaluation: &self.values[..self.domain.half_coset.size()],
                offset: d,
                step: coset.step_size / self.domain.half_coset.step_size,
            };
        }
        if let Some(d) = self.domain.half_coset.conjugate().find(coset.initial_index) {
            return CyclicSlice {
                evaluation: &self.values[self.domain.half_coset.size()..],
                offset: d,
                step: (-coset.step_size) / self.domain.half_coset.step_size,
...
impl<'a, F: ExtensionOf<BaseField>> Index<isize> for CyclicSlice<'a, F> {
    type Output = F;
...
impl<'a, F: ExtensionOf<BaseField>> Index<usize> for CyclicSlice<'a, F> {
    type Output = F;
...
mod tests {
    ...
    fn test_cyclic_slice() {
        ...
        for i in 0..coset.size() {
            assert_eq!(slice[i], circle_evaluation.get_at(coset.index_at(i)));
        }
    }
}
