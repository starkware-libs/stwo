...
use super::commitment_scheme::{CommitmentSchemeProof, TreeColumns, TreeVec};
use super::ColumnVec;
...
pub fn prove(
    ...
    let (mut open_points, _trace_oods_values) = air.mask_points_and_values(
        oods_point,
        &air.component_traces(&commitment_scheme.trees[0].polynomials),
    );

    // Add composiion poly points.
    open_points.push(vec![
        vec![oods_point],
        vec![oods_point],
        vec![oods_point],
        vec![oods_point],
    ...
    let commitment_scheme_proof =
        commitment_scheme.open_values(TreeColumns::new(open_points), channel);
...
pub fn verify(proof: StarkProof, air: &impl Air<CPUBackend>, channel: &mut Channel) -> bool {
    ...
    let oods_point = CirclePoint::<SecureField>::get_random_point(channel);
    let mut open_points = air.mask_points(oods_point);
    open_points.push(vec![
        vec![oods_point],
        vec![oods_point],
        vec![oods_point],
    ...
    let mut trace_oods_values = proof.commitment_scheme_proof.opened_values.0 .0.clone();
    let composition_oods_values = &trace_oods_values.pop().unwrap();
    ...
    commitment_scheme.verify_opening(
        TreeColumns::new(open_points),
