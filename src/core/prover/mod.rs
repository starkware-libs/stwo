...
use crate::core::commitment_scheme::{
    CommitmentSchemeProver, CommitmentSchemeVerifier, TreeColumns,
};
...
pub fn prove(
    ...
    // First tree - trace.
    let mut open_points = TreeColumns::new(vec![open_points.flatten()]);
...
pub fn verify(proof: StarkProof, air: &impl Air<CPUBackend>, channel: &mut Channel) -> bool {
    ...
    let flat_trace_values = &mut opened_values.0.pop().unwrap().into_iter();
