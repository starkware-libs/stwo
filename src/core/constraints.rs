...
mod tests {
    ...
    fn test_point_vanishing_degree() {
        ...
        for point in large_domain.iter() {
            let mut value =
                polynomial.eval_at_point(point) - polynomial.eval_at_point(vanish_point);
        ...
        // Check that the quotient polynomial is indeed of a degree higher by one than the original
        // polynomial.
        assert_eq!(quotient_polynomial.coeffs().len() as u32, large_domain_size);
        assert_ne!(
            quotient_polynomial.coeffs()[domain_size as usize],
            M31::zero()
        );
        for i in (domain_size + 1) as usize..large_domain_size as usize {
            assert_eq!(quotient_polynomial.coeffs()[i], M31::zero());
        }
    }
}
