...
impl<F: ExtensionOf<BaseField>, H: Hasher> FriProver<F, H, CommitmentPhase> {
    ...
    /// * An circle evaluation is not of a sufficiently low degree polynomial.
    ...
    fn commit_inner_layers(&mut self, mut evals: Vec<CircleEvaluation<F>>) -> LineEvaluation<F> {
        ...
        let first_layer_size = evals.last().map(folded_len).expect("no evaluation");
        let mut evaluation = LineEvaluation::new(vec![F::zero(); first_layer_size]);
...
pub struct FriVerifier<F: ExtensionOf<BaseField>, H: Hasher> {
    ...
    degree_bounds: Vec<LogCirclePolyDegreeBound>,
    proof: FriProof<F, H>,
}
...
impl<F: ExtensionOf<BaseField>, H: Hasher> FriVerifier<F, H> {
    ...
    /// # Panics
    ///
    /// Panics if `degree_bounds` is empty or if a degree bound is less than the last layer.
    pub fn new(
        config: FriConfig,
        proof: FriProof<F, H>,
        mut degree_bounds: Vec<LogCirclePolyDegreeBound>,
    ...
    pub fn verify(
        ...
        if circle_polynomials.len() != self.degree_bounds.len() {
            return Err(VerificationError::InvalidNumPolynomials {
                expected: self.degree_bounds.len(),
    ...
    fn verify_inner_layers(
        ...
        let mut degrees = self.degree_bounds.clone();
        ...
        for (i, layer) in self.proof.inner_layers.iter().enumerate() {
            ...
            let coset_evals = layer
                .coset_evals(&positions, &folded_positions, domain.size(), &evals)
...
/// Log folding factor for univariate polynomials.
...
pub struct FriLayerProof<F: ExtensionOf<BaseField>, H: Hasher> {
    ...
    ///
    /// The verifier is able to calculate some of the evaluations themselves. These are not stored
    /// in order to reduce the size of the proof. The subset stored here corresponds to the set of
    /// evaluations the verifier doesn't have but needs in order to verify the decommitment.
...
impl<F: ExtensionOf<BaseField>, H: Hasher> FriLayerProof<F, H> {
    ...
    /// should be the total number of evaluations (not cosets) in the entire layer.
    ///
    /// # Errors
    ///
...
/// # Panics
///
/// Panics if there are less than `FOLDING_FACTOR` many evaluations.
