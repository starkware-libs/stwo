...
use super::poly::line::{LineEvaluation, LinePoly, LinePolyOps};
...
use crate::core::backend::{Column, ColumnTrait};
...
pub trait FriOps: FieldOps<SecureField> + LinePolyOps<SecureField> {
    /// Folds a degree `d` polynomial into a degree `d/2` polynomial.
    ///
    /// Let `evals` be a polynomial evaluated on a [LineDomain] `E`, `alpha` be a random field
    ...
    fn fold_line(
        evals: &LineEvaluation<Self, SecureField, BitReversedOrder>,
        alpha: SecureField,
...
impl<B: FriOps, H: Hasher<NativeType = u8>> FriProver<B, H> {
    ...
    pub fn decommit(self, queries: &Queries) -> FriProof<H> {
...
pub struct FriVerifier<B: FriOps, H: Hasher> {
...
impl<B: FriOps, H: Hasher<NativeType = u8>> FriVerifier<B, H> {
    ...
    pub fn commit(
        ...
        Ok(Self {
            ...
            inner_layers,
            last_layer_domain,
            last_layer_poly: LinePoly::from_ordered_coefficients(Column::<B, _>::from_vec(
                last_layer_coeffs,
            )),
        })
    }
...
pub trait FriChannel {
    ...
    type Field: ExtensionOf<BaseField>;
...
struct SparseLineEvaluation {
    subline_evals: Vec<LineEvaluation<CPUBackend, SecureField, BitReversedOrder>>,
}
...
impl SparseLineEvaluation {
    ...
    ///
    /// Panics if the evaluation domain sizes don't equal the folding factor.
    fn new(subline_evals: Vec<LineEvaluation<CPUBackend, SecureField, BitReversedOrder>>) -> Self {
...
mod tests {
    ...
    type B = CPUBackend;
    type FriProver = super::FriProver<B, Blake2sHasher>;
    ...
    fn fold_line_works() {
        ...
        let odd_coeffs: [SecureField; DEGREE / 2] = [3, 5, 4, 1]
            .map(BaseField::from_u32_unchecked)
            .map(SecureField::from);
        let poly = LinePoly::<B, _>::new([even_coeffs, odd_coeffs].concat());
        let even_poly = LinePoly::<B, _>::new(even_coeffs.to_vec());
        let odd_poly = LinePoly::<B, _>::new(odd_coeffs.to_vec());
        ...
        let drp_evals = B::fold_line(&evals, alpha);
    ...
    fn fold_circle_to_line_works() {
        ...
        B::fold_circle_into_line(&mut folded_evaluation, &circle_evaluation, alpha);
    ...
    fn valid_proof_passes_verification() -> Result<(), VerificationError> {
        ...
        let verifier =
            FriVerifier::<B, _>::commit(&mut test_channel(), config, proof, bound).unwrap();
    ...
    fn valid_mixed_degree_proof_passes_verification() -> Result<(), VerificationError> {
        ...
        let verifier =
            FriVerifier::<B, _>::commit(&mut test_channel(), config, proof, bounds).unwrap();
    ...
    fn proof_with_removed_layer_fails_verification() {
        ...
        let verifier =
            FriVerifier::<B, _>::commit(&mut test_channel(), invalid_config, proof, bound);
    ...
    fn proof_with_invalid_last_layer_degree_fails_verification() {
        ...
        let verifier = FriVerifier::<B, _>::commit(&mut test_channel(), config, proof, bound);
    ...
    fn log_degree_bound<F: ExtensionOf<BaseField>>(
        polynomial: LineEvaluation<B, F, BitReversedOrder>,
    ) -> u32 {
