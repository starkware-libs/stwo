...
pub struct FriLayerProof<F: ExtensionOf<BaseField>, H: Hasher> {
    /// Subset of all coset evaluations.
    ///
...
impl<F: ExtensionOf<BaseField>, H: Hasher<NativeType = u8>> FriLayer<F, H> {
    ...
    fn decommit(self, queries: &Queries) -> FriLayerProof<F, H> {
        ...
        let evals_subset = queries
            // Group queries by the coset they reside in.
            .group_by(|a, b| a >> LOG_FOLDING_FACTOR == b >> LOG_FOLDING_FACTOR)
            .flat_map(|coset_positions| {
                const COSET_SIZE: usize = 1 << LOG_FOLDING_FACTOR;
                let coset_start = (coset_positions[0] / COSET_SIZE) * COSET_SIZE;
                let coset_end = coset_start + COSET_SIZE;
                let mut coset_evals =
                    <[F; COSET_SIZE]>::try_from(&self.evaluation[coset_start..coset_end])
                        .unwrap()
                        .map(Some);

                // Remove evals the verifier will be able to calculate.
                for position in coset_positions {
                    let coset_position = position % (1 << LOG_FOLDING_FACTOR);
                    coset_evals[coset_position] = None;
                }

                // Add coset indices to decommitment positions.
                decommit_positions.extend(coset_start..coset_end);
