...
impl<F: ExtensionOf<BaseField>, H: Hasher<NativeType = u8>> FriProver<F, H> {
    ...
    fn commit_inner_layers(
        ...
        let first_layer_size = columns[0].len() >> LOG_CIRCLE_TO_LINE_FOLDING_FACTOR;
        ...
        while layer_evaluation.len() > config.last_layer_domain_size() {
            ...
            while let Some(column) = columns.next_if(|c| c.len() > layer_evaluation.len()) {
...
impl<F: ExtensionOf<BaseField>, H: Hasher<NativeType = u8>> FriVerifier<F, H> {
    ...
    fn decommit_inner_layers(
        ...
        for layer in self.inner_layers.iter() {
            ...
            while column_bounds.next_if(|b| b > &layer.degree_bound).is_some() {
...
impl CirclePolyDegreeBound {
    ...
    fn fold(&self) -> LinePolyDegreeBound {
        LinePolyDegreeBound {
...
impl LinePolyDegreeBound {
    ...
    fn fold(self) -> Option<Self> {
        if self.log_degree_bound < LOG_FOLDING_FACTOR {
            return None;
        }

        let log_degree_bound = self.log_degree_bound - LOG_FOLDING_FACTOR;
...
// into evaluations on multiple smaller subgroups.
...
impl<F: ExtensionOf<BaseField>, H: Hasher<NativeType = u8>> FriLayerVerifier<F, H> {
    ...
    fn verify_and_fold(
        ...
        // TODO: When leaf values are removed from the decommitment, also remove this block.
        {
            ...
            if expected_decommitment_evals != actual_decommitment_evals {
                return Err(VerificationError::InnerLayerCommitmentInvalid { layer: self.index });
            }
        }
    ...
    fn extract_evaluation(
        ...
        let mut all_subgroup_evals = Vec::new();

        // Group queries by the subgroup they reside in.
        for subgroup_queries in queries.group_by(|a, b| a / SUBGROUP_SIZE == b / SUBGROUP_SIZE) {
            let subgroup_index = subgroup_queries[0] / SUBGROUP_SIZE;

            let subgroup_initial_index = bit_reverse_index(subgroup_index, self.domain.log_size());
            let subgroup_initial = self.domain.coset().index_at(subgroup_initial_index);
            let subgroup_domain = LineDomain::new(Coset::new(subgroup_initial, LOG_FOLDING_FACTOR));

            let mut subgroup_evals = Vec::<F>::new();
            let mut subgroup_queries = subgroup_queries.iter().peekable();

            // Insert the evals.
            for i in 0..SUBGROUP_SIZE {
                let eval_position = subgroup_index * SUBGROUP_SIZE + i;
                let eval = match subgroup_queries.next_if_eq(&&eval_position) {
                    Some(_) => evals_at_queries.next().unwrap(),
                    None => proof_evals.next().ok_or(
                        VerificationError::InnerLayerEvaluationsInvalid { layer: self.index },
                    )?,
                };

                subgroup_evals.push(eval);
            }

            all_subgroup_evals.push(LineEvaluation::new(subgroup_domain, subgroup_evals));
        }

        Ok(SparseLineEvaluation::new(all_subgroup_evals))
    }
}
...
impl<F: ExtensionOf<BaseField>, H: Hasher<NativeType = u8>> FriLayerProver<F, H> {
    ...
    /// Generates a decommitment of the subgroup evaluations at the specified positions.
    // TODO: Explain what the subgroups are better.
    fn decommit(self, queries: &Queries) -> FriLayerProof<F, H> {
        ...
        // Group queries by the subgroup they reside in.
        for query_group in queries.group_by(|a, b| a / SUBGROUP_SIZE == b / SUBGROUP_SIZE) {
            let subgroup_index = query_group[0] / SUBGROUP_SIZE;
            let mut subgroup_queries = query_group.iter().peekable();
            ...
            for i in 0..SUBGROUP_SIZE {
                ...
                if subgroup_queries.next_if_eq(&&eval_position).is_some() {
                    continue;
                }
