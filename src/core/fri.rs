...
impl<H: Hasher<NativeType = u8>> FriVerifier<H> {
    ...
    pub fn decommit<F>(
        self,
        queries: &Queries,
        decommited_values: Vec<SparseCircleEvaluation<B, F>>,
...
#[derive(Debug, Clone)]
pub struct SparseCircleEvaluation<B, F: ExtensionOf<BaseField>>
where
    B: FieldOps<F>,
{
...
impl<F: ExtensionOf<BaseField>> SparseCircleEvaluation<B, F>
where
    B: FieldOps<F>,
{
    /// # Panics
...
struct SparseLineEvaluation<F: ExtensionOf<BaseField>> {
    subline_evals: Vec<LineEvaluation<B, F, BitReversedOrder>>,
}
...
impl<F: ExtensionOf<BaseField>> SparseLineEvaluation<F> {
    ...
    ///
    /// Panics if the evaluation domain sizes don't equal the folding factor.
    fn new(subline_evals: Vec<LineEvaluation<B, F, BitReversedOrder>>) -> Self {
