...
trait ChannelHash {
    fn hash_digest(&self, seed: Self) -> Self;
    fn hash_counter(&self, counter: u32) -> Self;
}

impl ChannelHash for Blake2sHash {
    fn hash_digest(&self, seed: Blake2sHash) -> Self {
        Blake2sHasher::concat_and_hash(self, &seed)
    }

    fn hash_counter(&self, counter: u32) -> Blake2sHash {
...
impl<T: ChannelHash> Channel<T> {
    ...
    pub fn reseed(&self, seed: T) -> Self {
        let new_seed = self.digest.hash_digest(seed);
        Channel {
            digest: new_seed,
            counter: 0,
        }
    ...
    pub fn draw_random(&mut self) -> T {
        let rand = self.digest.hash_counter(self.counter);
        self.counter += 1;
        rand
    }
}

// TODO(ShaharS): Remove dead_code when channel is used.
#[cfg(test)]
mod tests {
    use super::Channel;
    ...
    fn test_channel() {
        ...
        let channel = channel.reseed(Blake2sHash::from(vec![1; 32]));
