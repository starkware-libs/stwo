...
//! `g(x_1, ..., x_n)` over the boolean hypercube. [`MultivariatePolyOracle`] provides methods for
//! evaluating sums and making transformations on `g` in the context of the protocol. It is intended
//! to be used in conjunction with [`prove_batch()`] to generate proofs.
...
/// Something that can be seen as a multivariate polynomial `g(x_1, ..., x_n)`.
...
///
/// Let the multivariate polynomials be `g_1, ..., g_n`. A single sum-check is performed on
/// multivariate polynomial `h = g_1 + lambda * g_2 + ... + lambda^(n-1) * g_n`. The `g_i`s do not
/// need to have the same number of variables. `g_i`s with less variables are folded in the latest
/// possible round of the protocol. For instance with `g_1(x, y, z)` and `g_2(x, y)` sum-check is
/// performed on `h(x, y, z) = g_1(x, y, z) + lambda * g_2(y, z)`. Claim `c_i` should equal the
/// claimed sum of `g_i(x_1, ..., x_j)` over all `(x_1, ..., x_j)` in `{0, 1}^{j}`.
///
...
/// Returns `p_1 + scalar * p_2 + ... + scalar^(n-1) * p_n`.
