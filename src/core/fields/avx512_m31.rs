...
impl M31AVX512 {
    #[inline(always)]
    /// Given x_1,...,x_K_BLOCK_SIZE values, each in [0, 2*P) packed in x, returns x_i in [0,P).
    ...
    #[inline(always)]
    /// Given x_1,...,x_K_BLOCK_SIZE values, each in [0, P^2) packed in x, returns x_i in [0,P).
