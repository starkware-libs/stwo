...
pub unsafe fn add_mod_p(a: __m512i, b: __m512i) -> __m512i {
    ...
    let unreduced_sum = _mm512_add_epi32(a, b);
    // Apply min(a, a-P) to each word.
    // When a in [P,2P], then a-P in [0,P] which is always less than [P,2P].
    // When a in [0,P-1], then a-P in [2^32-P,2^32-1] which is always greater than [0,P-1].
    _mm512_min_epu32(unreduced_sum, _mm512_sub_epi32(unreduced_sum, P))
}
...
pub unsafe fn sub_mod_p(a: __m512i, b: __m512i) -> __m512i {
    ...
    let unreduced_diff = _mm512_sub_epi32(a, b);
    // Apply min(a, a+P) to each word.
    // When a in [0,P], then a+P in [P,2P] which is always greater than [0,P].
    // When a in [2^32-P,2^32-1], then a+P in [0,P-1] which is always less than [2^32-P,2^32-1].
    _mm512_min_epu32(_mm512_add_epi32(unreduced_diff, P), unreduced_diff)
}
