...
use crate::core::backend::cpu::quotients::{alphas, column_line_coeffs, QuotientConstants};
...
pub fn accumulate_row_quotients(
    ...
    for (sample_batch, line_coeffs, alpha, denominator_inverses) in izip!(
        ...
        row_accumulator = row_accumulator * PackedSecureField::broadcast(*alpha)
...
fn denominator_inverses(
    ...
    let mut flat_denominator_inverses = flat_denominator_inverses.data.into_iter();

    sample_batches
        .iter()
        .map(|_| {
            (0..domain.size() / K_BLOCK_SIZE)
                .map(|_| flat_denominator_inverses.next().unwrap())
                .collect()
        })
        .collect()
}
