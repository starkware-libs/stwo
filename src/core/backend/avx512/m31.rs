...
impl PackedBaseField {
    ...
    /// # Safety
    ///
    /// This function is unsafe because it performs a load from a raw pointer. The caller must
    /// ensure that the pointer is valid and points to a memory location that contains a valid
    /// `i32`.
    ...
    /// # Safety
    ///
    /// This function is unsafe because it performs a store to a raw pointer. The caller must ensure
    /// that the pointer is valid and points to a memory location that can accommodate an `i32`.
