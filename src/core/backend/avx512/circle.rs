...
impl AVX512Backend {
    ...
    fn twiddle_steps<F: Field>(mappings: &[F]) -> Vec<F> {
        ...
        for i in 1..mappings.len() {
            let mut denominator = denominators[i - 1];
            denominator *= mappings[i];
...
impl PolyOps for AVX512Backend {
    ...
    fn eval_at_securefield_point(
        ...
        // 8 lowest mappings produce the first 2^8 twiddles. Seperate to optimize each calculation.
