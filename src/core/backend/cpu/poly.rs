...
impl<F: ExtensionOf<BaseField>> PolyOps<F> for CPUBackend {
    fn new_canonical_ordered(coset: CanonicCoset, values: Column<B, F>) -> CircleEvaluation<B, F> {
    ...
    fn interpolate(eval: CircleEvaluation<B, F>) -> CirclePoly<B, F> {
    ...
    fn extend(poly: &CirclePoly<B, F>, log_size: u32) -> CirclePoly<B, F> {
    ...
    fn evaluate(poly: &CirclePoly<B, F>, domain: CircleDomain) -> CircleEvaluation<B, F> {
