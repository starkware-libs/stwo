...
pub fn accumulate_row_quotients(
    ...
    for (sample_batch, line_coeffs, alpha, denominator_inverses) in izip!(
        sample_batches,
        ...
        row_accumulator = row_accumulator * *alpha + numerator * denominator_inverses[row];
    }
    row_accumulator
}
...
pub fn alphas(sample_batches: &[ColumnSampleBatch], random_coeff: SecureField) -> Vec<SecureField> {
    sample_batches
        .iter()
...
fn denominator_inverses(
    ...
    let mut flat_denominators = Vec::new();
    ...
    let mut flat_denominator_inverses = flat_denominator_inverses.chunks_mut(domain.size());

    sample_batches
        .iter()
        .map(|_| {
            let denominator_inverses = flat_denominator_inverses.next().unwrap();
