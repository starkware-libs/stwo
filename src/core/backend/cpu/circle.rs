...
// Group line domains:
// =============
// The line domain is formed by the x-projection of a size 2^(n+1) group. Itt is a set of 2^n + 1
...
// Ordering:
// =========
// Recall that a canonic line domain is the projection of only the ood points of a group. This is
// used in the canonic fft. It is ordered as the bit-reverse of
//   ((4k+1)G_{n+2}).x for k=0..2^n .
// Denote this as C_n.
...
// Combine using f(x) = f0(2x^2 - 1) + x f1(2x^2 - 1),
...
// Thus, we can simply "swap" the evaluation of f0 at -1 with the resulting evaluation at -1.
// This transforms the evaluation buffer to:
//   1, 0, C_1, C_2, ..., C_{n-1}, C_n
// and stores the evaluation on -1 instead of the las evaluation.
//
// Circle fft:
// ===========
...
//   (1,+-0),(0,+-1),(1/sqrt(2),+-1/sqrt(2)),...,
// This is all the points on the circle group except (-1,0)
