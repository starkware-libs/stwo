...
pub fn vecwise_butterflies(
    ...
    (val0, val1) = butterfly(val0, val1, t);
    ...
    (val0, val1) = val0.interleave(val1);
    (val0, val1) = butterfly(val0, val1, t1);

    (val0, val1) = val0.interleave(val1);
    (val0, val1) = butterfly(val0, val1, t0);
...
pub unsafe fn fft3(
    ...
    (val0, val4) = butterfly(val0, val4, u32x16::splat(twiddles_dbl2[0]));
    (val1, val5) = butterfly(val1, val5, u32x16::splat(twiddles_dbl2[0]));
    (val2, val6) = butterfly(val2, val6, u32x16::splat(twiddles_dbl2[0]));
    (val3, val7) = butterfly(val3, val7, u32x16::splat(twiddles_dbl2[0]));

    // Apply the second layer of butterflies.
    (val0, val2) = butterfly(val0, val2, u32x16::splat(twiddles_dbl1[0]));
    (val1, val3) = butterfly(val1, val3, u32x16::splat(twiddles_dbl1[0]));
    (val4, val6) = butterfly(val4, val6, u32x16::splat(twiddles_dbl1[1]));
    (val5, val7) = butterfly(val5, val7, u32x16::splat(twiddles_dbl1[1]));

    // Apply the first layer of butterflies.
    (val0, val1) = butterfly(val0, val1, u32x16::splat(twiddles_dbl0[0]));
    (val2, val3) = butterfly(val2, val3, u32x16::splat(twiddles_dbl0[1]));
    (val4, val5) = butterfly(val4, val5, u32x16::splat(twiddles_dbl0[2]));
    (val6, val7) = butterfly(val6, val7, u32x16::splat(twiddles_dbl0[3]));
...
mod tests {
    ...
    use crate::core::backend::simd::fft::rfft::{
        fft, fft_lower_with_vecwise, get_twiddle_dbls, vecwise_butterflies,
    };
    ...
    fn test_butterfly() {
        ...
        let (r0, r1) = butterfly(v0.into(), v1.into(), twiddle_dbl.into());
    ...
    fn test_fft_lower() {
        for log_size in 5..12 {
            ...
            assert_eq!(res.to_vec(), ground_truth_fft(domain, &values));
        }
    }
