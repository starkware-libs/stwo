...
pub struct PointEvaluationAccumulator {
    ...
    // Each `sub_accumulation` holds `sum_{i=0}^{n-1} evaluation_i * random_coeff^(n-1-i)`,
    ...
    pub fn finalize(self) -> QM31 {
        self.sub_accumulations
            ...
            .fold(QM31::default(), |total, (sub_accumulation, n_i)| {
                ...
                // with coefficients ranging from `random_coeff^0` to `random_coeff^(n_i-1)`.
                // We need to multiply `total` by `random_coeff^n_i` and add `sub_accumulation`.
...
/// Accumulates evalautions of the composition polynomial on the entire evaluation domain.
pub struct DomainEvaluationAccumulator {
    random_coeff: QM31,
    evaluations: Vec<Column>,
