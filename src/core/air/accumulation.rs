...
impl<B: Backend> DomainEvaluationAccumulator<B> {
    ...
    pub fn columns<const N: usize>(
        ...
        let mut component_random_coeff_offset: usize = 0;
        self.sub_accumulations
            ...
            .map(|(col, (_, n_cols))| {
                ...
                let accum = ColumnAccumulator {
                    component_offset: component_random_coeff_offset,
                    random_coeff_powers: random_coeffs,
                    col,
                };
                component_random_coeff_offset += n_cols;
                accum
            })
            .collect_vec()
            .try_into()
...
pub struct ColumnAccumulator<'a, B: Backend> {
    // Offset of the random coefficients for this component.
    component_offset: usize,
    random_coeff_powers: Vec<SecureField>,
    col: &'a mut SecureColumn<B>,
}
impl<'a> ColumnAccumulator<'a, CPUBackend> {
    /// Accumulate the columns in reverse order.
    pub fn accumulate(&mut self, col_index: usize, row_index: usize, evaluation: SecureField) {
        // Random coefficients are in regular order.
        let random_coeff_pow = self.random_coeff_powers
            [self.random_coeff_powers.len() - 1 - (col_index - self.component_offset)];
...
mod tests {
    ...
    fn test_domain_evaluation_accumulator() {
        ...
        for (evaluation_index, (log_size, evaluation)) in
            log_sizes.iter().zip(&evaluations).enumerate()
        {
