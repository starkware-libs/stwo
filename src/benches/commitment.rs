...
use std::{
    arch::x86_64::{
        ...
        _mm512_load_epi32, _mm512_mul_epi32, _mm512_or_epi32, _mm512_or_epi64, _mm512_set1_epi64,
...
#[inline(never)]
unsafe fn fft_outer<const FULL_REDUCE: bool>(
    values: *mut i32,
...
unsafe fn fft3<const I_OFF: usize, const FULL_REDUCE: bool>(
    values: *mut i32,
    ...
    let p3 = _mm512_set1_epi64(((1 << 31) - 1) * 3);
    ...
    for i_l in 0..(1 << I_OFF) {
        ...
        let (val0l, val4l) = butterfly::<false>(one, p3, mask, val0l, val4l, t6l);
        let (val1l, val5l) = butterfly::<false>(one, p3, mask, val1l, val5l, t6l);
        let (val2l, val6l) = butterfly::<false>(one, p3, mask, val2l, val6l, t6l);
        let (val3l, val7l) = butterfly::<false>(one, p3, mask, val3l, val7l, t6l);

        let (val0l, val2l) = butterfly::<false>(one, p3, mask, val0l, val2l, t4l);
        let (val1l, val3l) = butterfly::<false>(one, p3, mask, val1l, val3l, t5l);
        let (val4l, val6l) = butterfly::<false>(one, p3, mask, val4l, val6l, t4l);
        let (val5l, val7l) = butterfly::<false>(one, p3, mask, val5l, val7l, t5l);

        let (val0l, val1l) = butterfly::<FULL_REDUCE>(one, p3, mask, val0l, val1l, t0l);
        let (val2l, val3l) = butterfly::<FULL_REDUCE>(one, p3, mask, val2l, val3l, t1l);
        let (val4l, val5l) = butterfly::<FULL_REDUCE>(one, p3, mask, val4l, val5l, t2l);
        let (val6l, val7l) = butterfly::<FULL_REDUCE>(one, p3, mask, val6l, val7l, t3l);
        ...
        let (val0h, val4h) = butterfly::<false>(one, p3, mask, val0h, val4h, t6h);
        let (val1h, val5h) = butterfly::<false>(one, p3, mask, val1h, val5h, t6h);
        let (val2h, val6h) = butterfly::<false>(one, p3, mask, val2h, val6h, t6h);
        let (val3h, val7h) = butterfly::<false>(one, p3, mask, val3h, val7h, t6h);

        let (val0h, val2h) = butterfly::<false>(one, p3, mask, val0h, val2h, t4h);
        let (val1h, val3h) = butterfly::<false>(one, p3, mask, val1h, val3h, t5h);
        let (val4h, val6h) = butterfly::<false>(one, p3, mask, val4h, val6h, t4h);
        let (val5h, val7h) = butterfly::<false>(one, p3, mask, val5h, val7h, t5h);

        let (val0h, val1h) = butterfly::<FULL_REDUCE>(one, p3, mask, val0h, val1h, t0h);
        let (val2h, val3h) = butterfly::<FULL_REDUCE>(one, p3, mask, val2h, val3h, t1h);
        let (val4h, val5h) = butterfly::<FULL_REDUCE>(one, p3, mask, val4h, val5h, t2h);
        let (val6h, val7h) = butterfly::<FULL_REDUCE>(one, p3, mask, val6h, val7h, t3h);
...
unsafe fn reduce<const FULL_REDUCE: bool>(one: __m512i, mask: __m512i, x: __m512i) -> __m512i {
    if FULL_REDUCE {
        let q = _mm512_add_epi64(_mm512_add_epi64(_mm512_srli_epi64(x, 31), x), one);
        let q = _mm512_add_epi32(_mm512_srli_epi64(q, 31), x);
        _mm512_and_epi64(q, mask)
    } else {
...
unsafe fn butterfly<const FULL_REDUCE: bool>(
    ...
    */
    ...
    let m = reduce::<FULL_REDUCE>(one, mask, _mm512_mul_epi32(val1, twiddle));
    // m in [0, 3*2^31-4]

    // val0+m in [0, 4*2^31-5].
    let r0 = reduce::<FULL_REDUCE>(one, mask, _mm512_add_epi64(val0, m));
    // r0 in [0, 2^31+2].
    // val0+3p-m in [4,4*2^31+2].
    let r1 = reduce::<FULL_REDUCE>(one, mask, _mm512_sub_epi64(_mm512_add_epi64(val0, p3), m));
    // r1 in [0, 2^31+3].
...
pub fn fft_bench(
    ...
    (0..N_THR).into_par_iter().for_each(|i_thr| {
        ...
        unsafe { fft_outer::<false>(values_slice[..].as_mut_ptr() as *mut i32, twiddles, i_thr) };
    });
    (0..N_THR).into_par_iter().for_each(|i_thr| {
        // convert pointer back to a mutable slice.
        let values_slice = unsafe { unsafe_values.get().as_mut().unwrap() };
    ...
    (0..N_THR).into_par_iter().for_each(|i_thr| {
        // convert pointer back to a mutable slice.
        let values_slice = unsafe { unsafe_values.get().as_mut().unwrap() };
        unsafe { fft_outer::<true>(values_slice[..].as_mut_ptr() as *mut i32, twiddles, i_thr) };
    });
}
