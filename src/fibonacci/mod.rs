...
use crate::core::poly::circle::{CanonicCoset, CircleEvaluation, SecureValue};
...
pub struct FibonacciProof {
    ...
    pub composition_polynomial_column_oods_values: SecureValue,
    pub trace_opened_values: ColumnVec<Vec<BaseField>>,
    pub composition_polynomial_opened_values: Vec<Vec<BaseField>>,
...
impl Fibonacci {
    ...
    pub fn prove(&self) -> FibonacciProof {
        ...
        // polynomial.
        let mut oods_quotients =
            Vec::with_capacity(trace_oods_points.len() + SECURE_EXTENSION_DEGREE);
        ...
        for (evaluation, value) in zip(
            ...
            composition_polynomial_column_oods_values.0,
        ) {
            oods_quotients
...
pub fn verify_proof<const N_BITS: u32>(proof: FibonacciProof) -> bool {
    ...
    assert_eq!(
        composition_polynomial_oods_value,
        proof.composition_polynomial_column_oods_values.combine()
    );
    ...
    for (opened_values, oods_value) in zip(
        proof.composition_polynomial_opened_values,
        proof.composition_polynomial_column_oods_values.0,
    ) {
...
mod tests {
    ...
    fn test_prove_invalid_trace_value() {
        ...
        invalid_proof.trace_opened_values[0][4] += BaseField::one();
