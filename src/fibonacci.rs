...
impl Fibonacci {
    ...
    pub fn eval_quotient(&self, trace: impl PolyOracle) -> Field {
        // TODO(ShaharS): Change random coeffs to be random extension field elements.
        let random_coeffs = [
            Field::from_u32_unchecked(13489),
            Field::from_u32_unchecked(2213980),
            Field::from_u32_unchecked(312383),
        ];
        let mut quotient = random_coeffs[0] * self.eval_step_quotient(trace);
        quotient += random_coeffs[1] * self.eval_boundary_quotient(trace, 0, Field::one());
        quotient += random_coeffs[2]
