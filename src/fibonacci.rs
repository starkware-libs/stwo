...
pub struct AdditionalProofData {
    ...
    pub oods_quotients: Vec<CircleEvaluation<QM31>>,
}
...
mod tests {
    ...
    fn test_oods_quotients_are_low_degree() {
        ...
        let proof = fib.prove();
        let trace_quotients = proof.additional_proof_data.oods_quotients[0..3].to_vec();
        let composition_polynomial_quotient = proof.additional_proof_data.oods_quotients[3].clone();

        // Assert that the trace quotients are low degree.
        for quotient in trace_quotients.into_iter() {
            let interpolated_quotient_poly = quotient.interpolate();
        ...
        let interpolated_quotient_poly = composition_polynomial_quotient.interpolate();
        assert!(interpolated_quotient_poly.coeffs().len() <= (1 << FIB_LOG_SIZE) + 3);
    }
