...
use crate::core::queries::generate_queries;
...
impl Fibonacci {
    ...
    pub fn prove(&self) -> FibonacciProof {
        ...
        let domain_ratio =
            1 << (quotient_commitment_domain.n_bits - trace_commitment_domain.n_bits);
        let trace_queries = quotient_queries.iter().map(|q| q / domain_ratio).collect();
