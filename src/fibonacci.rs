...
impl<F: ExtensionOf<BaseField>, H: Hasher> CommitmentProof<F, H> {
    pub fn verify(&self, decommitment_positions: &[usize]) -> bool
    where
        F: IntoSlice<H::NativeType>,
    {
        self.decommitment
            .verify(self.commitment, decommitment_positions)
    }
}
...
impl Fibonacci {
    ...
    pub fn prove(&self) -> FibonacciProof {
        ...
        let trace_commitment =
            MerkleTree::<BaseField, MerkleHasher>::commit(vec![bit_reverse_vec(
                &trace_commitment_evaluation.values,
                trace_commitment_evaluation.domain.log_size(),
            )]);
        ...
        let composition_polynomial_commitment =
            MerkleTree::<QM31, MerkleHasher>::commit(vec![bit_reverse_vec(
...
pub fn verify_proof<const N_BITS: u32>(proof: &FibonacciProof) -> bool {
    ...
    assert!(proof
        .trace_commitment
        .verify(&trace_sparse_sub_circle_domain.to_decommitment_positions()));
    assert!(proof
        .composition_polynomial_commitment
        .verify(&composition_polynomial_sparse_sub_circle_domain.to_decommitment_positions()));
    true
}
