...
impl Fibonacci {
    ...
    pub fn eval_composition_polynomial<F: ExtensionOf<BaseField>, EF: ExtensionOf<F>>(
        ...
        let mut composition_polynomial = random_coeff.pow(0) * self.eval_step_quotient(trace);
        composition_polynomial +=
            random_coeff.pow(1) * self.eval_boundary_quotient(trace, 0, BaseField::one());
        composition_polynomial += random_coeff.pow(2)
    ...
    /// Returns the composition_polynomial values using the trace and a random coefficient.
    ...
    pub fn prove(&self) -> FibonacciProof {
        ...
        let composition_polynomial_decommitment =
            composition_polynomial_merkle.generate_decommitment(&composition_polynomial_queries);
