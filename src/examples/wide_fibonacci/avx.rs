...
impl Component<AVX512Backend> for WideFibComponent {
    ...
    fn evaluate_constraint_quotients_on_domain(
        ...
        for vec_row in 0..(1 << (eval_domain.log_size() - VECS_LOG_SIZE as u32)) {
            ...
            let mut row_res = PackedSecureField::from_packed_m31s([
                a - PackedBaseField::one(),
                PackedBaseField::zero(),
                PackedBaseField::zero(),
                PackedBaseField::zero(),
            ]) * PackedSecureField::broadcast(accum.random_coeff_powers[0]);
            ...
            for i in 0..(N_COLS - 2) {
                unsafe {
                    ...
                    row_res += PackedSecureField::broadcast(accum.random_coeff_powers[i + 1])
                        * (a_sq + b_sq - c);
            ...
            accum.col.set_packed(
                ...
                accum.col.packed_at(vec_row)
                    + row_res * PackedBaseField::broadcast(denom_inverses.at(vec_row)),
            )
        }
    }
