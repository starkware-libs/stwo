...
impl Component<CPUBackend> for WideFibComponent {
    ...
    fn evaluate_constraint_quotients_on_domain(
        ...
        for point in eval_domain.iter() {
            ...
            let mut boundary_denom = BaseField::one();
            for instance in 0..(1 << self.log_n_instances) {
                boundary_denom *= point_vanishing(
                    step_zero_domain.at(bit_reverse_index(instance, self.log_column_size())),
                    point,
                );
            }
            boundary_denoms.push(boundary_denom);
        }
    ...
    fn evaluate_constraint_quotients_at_point(
        ...
        let mut boundary_denominator = SecureField::one();
        for instance in 0..(1 << self.log_n_instances) {
            boundary_denominator *= point_vanishing(
                step_zero_domain.at(bit_reverse_index(instance, self.log_column_size())),
                point,
            );
        }
