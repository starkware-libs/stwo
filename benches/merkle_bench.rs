...
fn merkle_blake3_in_place_benchmark(c: &mut Criterion) {
    ...
    for exp in 15u32..20u32 {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, &_size| {
            b.iter(|| merkle_tree::MerkleTree::<Blake3Hasher>::root_without_caching(&leaves))
        });
    }
    group.finish();
}
...
fn merkle_blake2s_benchmark(c: &mut Criterion) {
    ...
    for exp in 15u32..20u32 {
        ...
        group.throughput(Throughput::Bytes((size * 8) as u64));
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, &_size| {
            b.iter(|| merkle_tree::MerkleTree::<Blake2sHasher>::root_without_caching(&leaves))
        });
    }
    group.finish();
}
...
fn compare_blakes(c: &mut Criterion) {
    ...
    for exp in 15u32..20u32 {
        ...
        let size = 2usize.pow(exp);
        let leaves: Vec<u32> = (0..(size as u32)).collect();
        let mut tree: MerkleTree<Blake3Hasher> = MerkleTree::from_leaves(leaves.clone());
        group.sample_size(10);
        group.throughput(Throughput::Elements(size as u64));
        group.bench_with_input(BenchmarkId::new("Blake2s256 in-place", size), &size, |b, &_size| {
            b.iter(|| merkle_tree::MerkleTree::<Blake2sHasher>::root_without_caching(&leaves))
        });
        group.bench_with_input(BenchmarkId::new("Blake3 in-place", size), &size, |b, &_size| {
            b.iter(|| merkle_tree::MerkleTree::<Blake3Hasher>::root_without_caching(&leaves))
        });
        group.bench_with_input(BenchmarkId::new("Blake3 with layer caching", size), &size, |b, &_size| {
            b.iter(|| tree.commit())
        });
    }
    group.finish();
...
fn single_blake2_hash_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("Single blake2s hash",|b|{ 
        b.iter_batched(|| ->  Blake2s256{Blake2s256::new()},
                             |mut h|{ h.update(&input[..]); h.finalize()},
...
fn single_blake3_hash_benchmark(c: &mut Criterion) {
    // let mut group = c.benchmark_group("Single Hash");
    let input = [0u8;1];
    c.bench_function("Single blake3 hash",|b|{ 
