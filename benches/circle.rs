...
pub fn circle_point_bench(c: &mut criterion::Criterion) {
    ...
    let mut index_state: [CirclePointIndex; N_STATE_ELEMENTS] = [(); N_STATE_ELEMENTS]
        .map(|_| CirclePointIndex(rng.gen::<usize>() % (1 << M31_CIRCLE_ORDER_BITS)));
    ...
    for (index, point) in index_state.iter().zip(point_state.iter()) {
