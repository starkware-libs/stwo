...
//! ahead of this host-delegate. Absent such a kernel, this audited host-delegate is the ONLY
//! correct path and the DEFAULT.
//!
...
//!
//! 1. A generic `GpuDomainEvaluator` against `CudaBackend` device columns is UNSOUND: its `off ==
//!    0` fast path calls `VeryPackedBaseColumn::transform_under_ref(col)`, which reinterprets
//!    *host* SIMD memory. A `CudaBackend` `BaseFieldVec` is a raw device pointer, so that transmute
//!    is undefined behavior; its non-zero-offset path would also issue one single-element D2H copy
...
use super::component_prover::{
    ConstraintQuotientInputs, accumulate_pointwise_cpu, get_constraint_quotient_inputs,
};
use super::cuda_constraint_kernel::{
    GpuConstraintDispatch, registered_expected_kernel_guard, registered_gpu_constraint_kernel,
};
...
///
/// UNCONDITIONAL for a guarded MAIN: the former `CUDA_CONSTRAINT_CPU_FALLBACK` /
/// `CUDA_GPU_CONSTRAINTS` escape hatches are removed — a guarded MAIN falling to the host delegate
/// is always a hard error. This function is only reachable from `ComponentProver<CudaBackend>`, so
/// the CPU/SimdBackend build never calls it; the GPU-kernel == host-delegate byte-identity check is
/// now the T6 test, which compares the CudaBackend (GPU kernel) proof against the SimdBackend
/// (host) proof.
fn panic_if_main_host_delegate(n_constraints: usize, log_n_rows: u32) {
    ...
    panic!(
        "A plugin-declared MAIN component (expected-kernel guard matched) fell to the audited \
         ...
         engage) — composition_eval would run ~60x slower, benchmarking the WRONG path. The \
         kernel either was not registered (plugin `register()`), declined on a structural \
         mismatch (the kernel's own decline-guard constants stale vs the AIR), or its drawn \
         relation was not installed. Refusing to silently fall back. \
...
fn poly_to_cpu(poly: &Poly<CudaBackend>) -> Poly<CpuBackend> {
    ...
    let coeffs =
        poly.coeffs.as_ref().map(|c| CircleCoefficients::<CpuBackend>::new(c.coeffs.to_cpu()));
...
fn build_scoped_host_polys<E: FrameworkEval>(
    ...
    let mut needed: Vec<Vec<bool>> =
        device_polys.iter().map(|tree| vec![false; tree.len()]).collect();

    for location in component.trace_locations() {
        needed[location.tree_index][location.col_start..location.col_end].fill(true);
    }
        ...
        device_polys
            ...
            .map(|(tree_index, tree)| {
                tree.iter()
                    .enumerate()
                    .map(|(col, poly)| {
                        if needed[tree_index][col] { poly_to_cpu(poly) } else { dummy_host_poly() }
                    })
                    .collect()
            })
            .collect(),
...
impl<E: FrameworkEval + Sync> ComponentProver<CudaBackend> for FrameworkComponent<E> {
    fn evaluate_constraint_quotients_on_domain(
        ...
        // host-delegate below. The host-delegate code below is unchanged and remains the
        // fallthrough for no-kernel/generic components. No host constraint-eval math is
        ...
        // Reaching here means this component takes the audited host-delegate. Normal for small
        // table components; a hard error for a plugin-declared MAIN component (see fn doc
        // for the rationale).
        ...
        let cpu_trace = Trace { polys: cpu_polys.as_cols_ref() };
        ...
        // host->SIMD repack (`BaseColumn::from_cpu` /
        // `SecureColumnByCoords::<SimdBackend>::from_cpu`) is positional (logical index `i`
        // -> packed `i / N_LANES`, lane `i % N_LANES`) and order-preserving, so the
        // eval-domain trace columns and the seed carry the same logical bit-reversed order
        // as the `CpuBackend` values.
        ...
        *accum.col =
            SecureColumnByCoords { columns: host_result.columns.map(|c| c.into_iter().collect()) };
    }
}
