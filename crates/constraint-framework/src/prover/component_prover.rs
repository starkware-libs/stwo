...
impl<E: FrameworkEval + Sync> ComponentProver<SimdBackend> for FrameworkComponent<E> {
    fn evaluate_constraint_quotients_on_domain(
        ...
        let mut den_inverses = (0..1 << log_expand)
            .map(|i| coset_vanishing(trace_domain.coset(), eval_domain.at(i)).inverse())
            .collect_vec();
        bit_reverse(&mut den_inverses);
        ...
        iter.for_each(|(chunk_idx, mut chunk)| {
            ...
            for idx_in_chunk in 0..CHUNK_SIZE {
                ...
                // Finalize row.
                unsafe {
                    let den_inverse = VeryPackedBaseField::broadcast(
                        den_inverses[vec_row
                            >> (trace_domain.log_size() - LOG_N_LANES - LOG_N_VERY_PACKED_ELEMS)],
                    );
                    chunk.set_packed(
                        idx_in_chunk,
                        chunk.packed_at(idx_in_chunk) + row_res * den_inverse,
                    )
                }
            }
        });
...
fn accumulate_pointwise_cpu<E: FrameworkEval>(
    ...
    for row in 0..(1 << eval_log_size) {
        ...
        // Finalize row.
        let denom_inv = den_inverses[row >> trace_log_size];
        res.set(row, accum.at(row) + row_res * denom_inv)
    }
    res
}
