...
mod tests {
    ...
    fn test_wide_fib_with_unused_pp_and_hardcoded_lifting() {
        for log_n_instances in 4..=8 {
            ...
            config.lifting_log_size =
                LiftingLogSize::Fixed(log_size_unused_pp + config.fri_config.log_blowup_factor);
            // Precompute twiddles.
            let twiddles = SimdBackend::precompute_twiddles(
    ...
    #[test_log::test]
    /// Tests dynamic (`AtLeast`) lifting: the floor sits above the trace size (so `AtLeast` raises
    /// the small trace tree to it), while the larger unused preprocessed column still drives its
    /// own tree's lifting above the floor — i.e. the lifting size is determined dynamically per
    /// tree rather than hardcoded.
    fn test_wide_fib_with_unused_pp_and_dynamic_lifting() {
        for log_n_instances in 4..=8 {
            let mut config = PcsConfig::default();
            let log_size_unused_pp = log_n_instances + 3;
            // Floor strictly between the trace size and the large preprocessed column (after LDE):
            // `AtLeast` raises the trace tree up to this floor, and lets the preprocessed tree grow
            // past it to fit its largest column.
            config.lifting_log_size =
                LiftingLogSize::AtLeast(log_n_instances + 1 + config.fri_config.log_blowup_factor);
            // Precompute twiddles.
            let twiddles = SimdBackend::precompute_twiddles(
                CanonicCoset::new(log_size_unused_pp + 1 + config.fri_config.log_blowup_factor)
                    .circle_domain()
