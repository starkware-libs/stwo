...
///     });
///
/// let expected_first_3_rows = [[0,1,1], [1,2,5], [2,3,13]].map(|row| row.map(M31::from));
/// let first_3_rows = [0,1,2].map(|i| trace.row_at(i));
///
/// assert_eq!(first_3_rows, expected_first_3_rows);
/// ```
#[derive(Debug)]
pub struct IterableTrace<const N: usize> {
    data: [Vec<PackedM31>; N],

    /// Log number of M31 elements in each column.
    log_size: u32,
}
