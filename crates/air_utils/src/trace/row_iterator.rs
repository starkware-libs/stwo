    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let item: Vec<&mut PackedM31> = self
            ...
            .map(|i| unsafe {
                ...
                let (head, tail) = i.split_at_mut(1);
                *i = tail;
                &mut (*head)[0]
            })
            .collect_vec();
...
impl DoubleEndedIterator for RowIterMut<'_> {
    fn next_back(&mut self) -> Option<Self::Item> {
        ...
        let item: Vec<&mut PackedM31> = self
            ...
            .map(|i| unsafe {
                // SAFETY: The self.v contract ensures that any split_at_mut is valid.
                let (head, tail) = i.split_at_mut(i.len() - 1);
                *i = head;
                &mut (*tail)[0]
            })
            .collect_vec();
...
impl<'trace> Producer for RowProducer<'trace> {
    ...
    fn split_at(self, index: usize) -> (Self, Self) {
        let mut left: Vec<_> = Vec::with_capacity(self.data.len());
        let mut right: Vec<_> = Vec::with_capacity(self.data.len());
        unsafe {
            left.set_len(self.data.len());
            right.set_len(self.data.len());
        }
