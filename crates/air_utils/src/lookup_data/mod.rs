...
mod tests {
    ...
    fn test_derived_lookup_data_iter_mut() {
        ...
        let (expected_field0, expected_field1, expected_field2): (Vec<_>, Vec<_>, Vec<_>) = arr
        ...
        assert!(
            fields_are_equal(expected_field0, actual.0),
            "Failed on Vec<PackedM31>"
        );
        assert!(
            fields_are_equal(expected_field1, actual.1),
            "Failed on Vec<[PackedM31; 2]>"
        );
        assert!(
            fields_are_equal(expected_field2, actual.2),
            "Failed on [Vec<[PackedM31; 2]>; 2]"
        );
    }

    fn fields_are_equal<const N: usize>(a: Vec<[PackedM31; N]>, b: Vec<[PackedM31; N]>) -> bool {
        assert_eq!(a.len(), b.len());
        all(a.iter().zip(b.iter()), |(a, b)| {
            a.into_iter().flat_map(|v| v.to_array()).collect_vec()
                == b.into_iter().flat_map(|v| v.to_array()).collect_vec()
        })
    }
}
