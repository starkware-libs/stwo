...
impl TraceLocationAllocator {
    ...
    pub fn validate_preprocessed_columns(
        ...
        for (column, idx) in self.preprocessed_columns.iter() {
            if let Some(preprocessed_column) = preprocessed_columns.get(*idx) {
                assert_eq!(column.to_string(), preprocessed_column.to_string());
            } else {
                panic!(
                    "Preprocessed column {:?} is missing from preprocessed_columns",
                    column
                );
            }
        }
