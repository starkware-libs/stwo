    ...
    fn next_for_structure<T>(&mut self, structure: &TreeVec<ColumnVec<T>>) -> TreeVec<TreeSubspan> {
        ...
        TreeVec::new(
            zip(&mut *self.next_owned_tree_offsets, &**structure)
                .enumerate()
                .take_while(|(tree_index, (..))| *tree_index != 2)
...
impl<E: FrameworkEval> Component for FrameworkComponent<E> {
    ...
    fn evaluate_constraint_quotients_at_point(
        ...
        let owned_mask = mask.sub_tree(&self.owned_trace_locations);
        let static_mask = mask.sub_tree_single_columns(&self.static_columns_locations);
        let mask = TreeVec::concat_cols([owned_mask, static_mask].into_iter());
...
impl<E: FrameworkEval> ComponentProver<SimdBackend> for FrameworkComponent<E> {
    fn evaluate_constraint_quotients_on_domain(
        ...
        > = if need_to_extend {
            ...
            println!("something");
            let s = component_polys.as_cols_ref().map_cols(|col| {
                println!("{}. {}", col.log_size(), eval_domain.log_size());
