...
macro_rules! logup_proxy {
    () => {
        ...
        fn finalize_logup_batched(&mut self, batching: &[usize]) {
            ...
            let keys_set: std::collections::HashSet<usize> =
                fracs_by_batch.keys().cloned().collect();
            let all_batches_set: std::collections::HashSet<usize> = (0..last_batch + 1).collect();
            ...
            // All except the last batch.
            for batch_id in (0..last_batch) {
                let cur_frac: Fraction<Self::EF, Self::EF> =
                    fracs_by_batch[&batch_id].clone().into_iter().sum();
                let [cur_cumsum] =
            ...
            let frac: Fraction<Self::EF, Self::EF> =
                fracs_by_batch[&last_batch].clone().into_iter().sum();
        ...
        fn finalize_logup(&mut self) {
            let batches: Vec<_> = (0..self.logup.fracs.len()).collect();
        ...
        fn finalize_logup_in_pairs(&mut self) {
            let batches: Vec<_> = (0..self.logup.fracs.len()).map(|n| n / 2).collect();
            self.finalize_logup_batched(&batches)
        }
    };
}
