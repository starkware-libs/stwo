...
pub enum PreprocessedColumn {
    /// A column with a single one at the first position, and zeros elsewhere.
    IsFirst(u32),
    Plonk(usize),
    /// A column with sequence of numbers from 0 to 2^log_size - 1.
    Seq(u32),
...
mod tests {
    ...
    fn test_packed_at_seq() {
        ...
        let expected_seq = std::array::from_fn::<_, { 1 << LOG_SIZE }, _>(|i| M31::from(i as u32));
