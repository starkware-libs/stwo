...
// (1 << 62) - 1 is an offset no column can reach, it signifies the variable
// offset, which is an input to the verifier.
const CLAIMED_SUM_DUMMY_OFFSET: usize = (1 << 62) - 1;
...
impl FormalLogupAtRow {
    pub fn new(interaction: usize, has_partial_sum: bool, log_size: u32) -> Self {
        ...
        Self {
            ...
            claimed_sum: if has_partial_sum {
                Some((Expr::Param(claimed_sum_name), CLAIMED_SUM_DUMMY_OFFSET))
            } else {
                None
            },
