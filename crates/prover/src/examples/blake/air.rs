...
use crate::examples::blake::scheduler::{self, BlakeInput};
use crate::examples::blake::{round, xor_table, BlakeXorElements, XorAccums, N_ROUNDS};
...
#[allow(unused)]
pub fn prove_blake(log_size: u32) -> (BlakeAir, StarkProof<Blake2sMerkleHasher>) {
    ...
    // Split round inputs to components, of size 8 and 2.
...
mod tests {
    ...
    // Note: this test is slow. Only use for becnhmarks, in release.
    #[ignore]
    #[test_log::test]
    fn test_simd_blake_prove() {
        ...
        // Draw lookup element.
        let _blake_lookup_elements = RoundElements::draw(channel);
        let _round_lookup_elements = RoundElements::draw(channel);
        let _xor_lookup_elements = XorElements::draw(channel);
