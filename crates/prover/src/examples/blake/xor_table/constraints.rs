    ...
    ($modname:tt, $elements:tt, $elem_bits:literal, $expand_bits:literal) => {
            ...
            XorTableEvalAtRow<'a, E, ELEM_BITS, EXPAND_BITS>
        {
            pub fn eval(mut self) -> E {
                ...
                let al = self.eval.get_preprocessed_column(PreprocessedColumn::XorTable(
                    ELEM_BITS,
                    EXPAND_BITS,
                    0,
                ));

                let bl = self.eval.get_preprocessed_column(PreprocessedColumn::XorTable(
                    ELEM_BITS,
                    EXPAND_BITS,
                    1,
                ));

                let cl = self.eval.get_preprocessed_column(PreprocessedColumn::XorTable(
                    ELEM_BITS,
                    EXPAND_BITS,
                    2,
                ));
