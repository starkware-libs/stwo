...
use crate::constraint_framework::RelationType;
...
macro_rules! xor_table_gen {
    ($modname:tt, $elements:tt, $elem_bits:literal, $expand_bits:literal) => {
        pub mod $modname {
            use super::*;
            use crate::examples::blake::xor_table::$modname::{
                column_bits, limb_bits, XorAccumulator,
            };
            ...
            pub fn generate_interaction_trace<
                ...
                X: RelationType<PackedBaseField, PackedSecureField>,
            >(
                ...
                let mut logup_gen =
                    LogupTraceGenerator::new(column_bits::<ELEM_BITS, EXPAND_BITS>());
                ...
                for [(i0, mults0), (i1, mults1)] in &mut iter {
                    ...
                    for vec_row in 0..(1 << (column_bits::<ELEM_BITS, EXPAND_BITS>() - LOG_N_LANES))
                    {
                        ...
                        let p0: PackedSecureField = lookup_elements.combine(
                            &[a0, b0, c0]
                                .map(|x| unsafe { PackedBaseField::from_simd_unchecked(x) }),
                        );
                ...
                if let Some(rem) = iter.into_remainder() {
                    if let Some((i, mults)) = rem.collect_vec().pop() {
                        ...
                        #[allow(clippy::needless_range_loop)]
                        for vec_row in
                            0..(1 << (column_bits::<ELEM_BITS, EXPAND_BITS>() - LOG_N_LANES))
                        {
                            ...
                            let b = u32x16::splat((bh << limb_bits) | (bm << LOG_N_LANES))
                                | offsets_vec;
...
xor_table_gen!(xor9, XorElements9, 9, 2);
xor_table_gen!(xor8, XorElements8, 8, 2);
xor_table_gen!(xor7, XorElements7, 7, 2);
xor_table_gen!(xor4, XorElements4, 4, 0);
