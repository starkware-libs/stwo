...
impl ComponentTraceGenerator<CpuBackend> for FibonacciTraceGenerator {
    ...
    fn write_trace(
        ...
        let component_trace_generator = registry.get_generator_mut::<Self>(component_id);
        assert!(
            component_trace_generator.inputs_set(),
            "Fibonacci input not set."
        );
        let trace_domain = CanonicCoset::new(component_trace_generator.log_size.unwrap());
