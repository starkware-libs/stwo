...
mod tests {
    ...
    use crate::constraint_framework::preprocessed_columns::{
        IsFirst, IsStepWithOffset, PreprocessedColumn,
    };
    ...
    fn test_mle_eval_constraints_with_log_size_5() {
        ...
        let is_first_col = [IsFirst::new(log_size).gen_preprocessed_column_simd()];
    ...
    fn eq_constraints_with_4_variables() {
        ...
        let carry_quotients_col = gen_carry_quotient_col(&eval_point).into_coordinate_evals();
        let is_first_col = [IsFirst::new(N_VARIABLES as u32).gen_preprocessed_column_simd()];
        let aux_trace = chain![carry_quotients_col, is_first_col].collect();
    ...
    fn eval_step_selector_with_offset_works() {
        ...
        let coset = CanonicCoset::new(LOG_SIZE).coset();
        let col_eval =
            IsStepWithOffset::new(LOG_SIZE, LOG_STEP, OFFSET).gen_preprocessed_column_simd();
