...
use super::eq_eval_constraints::{EqEvalsMask, MleEvalPoint};
use crate::constraint_framework::EvalAtRow;
use crate::core::fields::qm31::SecureField;
use crate::examples::xor::prefix_sum_constraints::PrefixSumMask;

pub struct MleEvalMask<E: EvalAtRow, const N_VARIABLES: usize> {
    /// Mask item on the column that interpolates all MLE coefficients in the lagrange basis.
    mle_coeff: E::EF,
    eq_evals_mask: EqEvalsMask<E, N_VARIABLES>,
    prefix_sum_mask: PrefixSumMask<E>,
}

impl<E: EvalAtRow, const N_VARIABLES: usize> MleEvalMask<E, N_VARIABLES> {
    pub fn eval(
        self,
        eval: &mut E,
        mle_eval_point: MleEvalPoint<N_VARIABLES>,
        claim_shift: SecureField,
    ) {
        self.eq_evals_mask.eval(eval, mle_eval_point);
        let mle_term = self.mle_coeff * self.eq_evals_mask.curr;
        self.prefix_sum_mask.eval(eval, mle_term, claim_shift);
    }
}
...
mod tests {
    ...
    use crate::examples::xor::eq_eval_constraints::{self, EqEvalsMask, MleEvalPoint};
    use crate::examples::xor::mle_eval_constraints::MleEvalMask;
    use crate::examples::xor::prefix_sum_constraints::PrefixSumMask;
    ...
    fn test_mle_eval_constraints_with_log_size_5() {
        ...
        assert_constraints(&trace_polys, trace_domain, |mut eval| {
            ...
            let eq_evals_mask = EqEvalsMask::draw::<EVAL_TRACE, CONST_TRACE>(&mut eval);
            let prefix_sum_mask = PrefixSumMask::draw::<EVAL_TRACE>(&mut eval);
            let mle_eval_mask = MleEvalMask {
                mle_coeff,
                eq_evals_mask,
                prefix_sum_mask,
            };
            mle_eval_mask.eval(&mut eval, mle_eval_point, claim_shift);
        });
    }
