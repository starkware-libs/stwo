...
pub fn prove_poseidon(
    ...
    let mut commitment_scheme =
        CommitmentSchemeProver::<WebBackend, Blake2sMerkleChannel>::new(config, &twiddles);
    ...
    let trace: ColumnVec<CircleEvaluation<WebBackend, BaseField, BitReversedOrder>> = trace.into();
    let mut tree_builder = commitment_scheme.tree_builder();
