...
mod tests {
    ...
    pub fn assert_constraints_on_lookup_column(
        ...
        for (i, cell) in column.iter().enumerate() {
            assert_eq!(
                *cell
                ...
                secure_combination(&[input_trace[0][i], input_trace[1][i]], alpha, z) * prev_value
            );
            prev_value = *cell;
        }
        ...
        assert_eq!(
            ...
            (secure_combination(&[input_trace[0][0], input_trace[1][0]], alpha, z))
        );
    }
    ...
    fn test_composition_is_low_degree() {
        ...
        let interaction_elements = &[m31!(43), m31!(44)];
        let interaction_trace = wide_fib.interact(&trace.iter().collect(), interaction_elements);
