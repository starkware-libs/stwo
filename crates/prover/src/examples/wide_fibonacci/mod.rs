...
mod tests {
    ...
    const FIB_SEQUENCE_LENGTH: usize = 8;
    ...
    fn test_wide_fib_prove_with_poseidon() {
        ...
        let config = PcsConfig {
            pow_bits: 10,
            fri_config: FriConfig::new(4, 4, 15),
        };
        // Precompute twiddles.
        let twiddles = SimdBackend::precompute_twiddles(
            CanonicCoset::new(LOG_N_INSTANCES + 1 + config.fri_config.log_blowup_factor)
        ...
        println!("Setup took: {:?}", now.elapsed());
        ...
        serialize_proof_cairo1_array(&proof);
    ...
    fn serialize_proof_cairo1_array(proof: &StarkProof<Poseidon252MerkleHasher>) {
