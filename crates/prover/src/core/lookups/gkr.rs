...
use super::utils::{eq, fold_mle_evals};
...
pub trait GkrBinaryLayer: Sized {
    ...
    /// representing `c_i` in the next layer. These polynomials must be combined with `lambda` into
    /// a single polynomial `h = g_0 + lambda * g_1 + ... + lambda^(n-1) * g_{n-1}`. The oracle for
    /// `h` should be returned.
...
pub enum GkrError {
    ...
    MalformedProof,
    #[error("provided an invalid number of components (given {given} but proof expects {proof})")]
