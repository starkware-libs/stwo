...
/// Stores evaluations of [`eq(x, y)`] for all values of `x = (x_0, ..., x_{n-1})` where `x_0 = 0`
/// and `x_1, ..., x_{n-1}` in `{0, 1}`.
///
...
pub trait GkrLayer: Sized {
    ...
    /// into a single polynomial `h = g_0 + lambda * g_1 + ... + lambda^(n-1) * g_{n-1}`. The oracle
    /// for `h` should be returned.
...
pub fn partially_verify_batch(
    ...
    for (layer, sumcheck_proof) in sumcheck_proofs.iter().enumerate() {
        ...
        for &component in &sumcheck_components {
            ...
            let folded_output = horner_eval(&circuit_output, component_lambda);
