...
// TODO: Remove trait in favour of enum once Grand Product and LogUp trace added.
...
pub fn prove_batch<L: GkrBinaryLayer>(
    ...
    let n_instances = top_layers.len();
    let n_layers_by_instance = top_layers.iter().map(|l| l.n_variables()).collect_vec();
    let n_layers = top_layers.iter().map(|l| l.n_variables()).max().unwrap();

    // Evaluate all instance circuits and collect the layer values.
    let mut layers_by_instance = top_layers
        .into_iter()
    ...
    for layer in 0..n_layers {
        ...
        for (instance, layers) in layers_by_instance.iter_mut().enumerate() {
            if n_layers_by_instance[instance] == n_remaining_layers {
                ...
                output_claims_by_instance[instance] = Some(output_layer.clone());
                claims_to_verify_by_instance[instance] = Some(output_layer);
            }
        }
        ...
        for (instance, claims_to_verify) in claims_to_verify_by_instance.iter().enumerate() {
            if let Some(claims_to_verify) = claims_to_verify {
                let layer = match layers_by_instance[instance].next().unwrap() {
    ...
    let artifact = GkrArtifact {
        ...
        n_variables_by_instance: (0..n_instances).map(|i| n_layers_by_instance[i]).collect(),
    };
...
/// point and claimed evaluations in the top layer columns for each instance at the OOD point.
/// These claimed evaluations are not checked in this function - hence partial verification.
pub fn partially_verify_batch(
    gate_by_instance: Vec<&dyn GkrBinaryGate>,
    proof: &GkrBatchProof,
    channel: &mut impl Channel,
    ...
    for (layer, sumcheck_proof) in sumcheck_proofs.iter().enumerate() {
        ...
        for &instance in &sumcheck_instances {
            ...
            let gate_output = gate_by_instance[instance].eval(&row0, &row1);
            let folded_output = horner_eval(&gate_output, instance_lambda);
...
pub struct GkrArtifact {
    ...
    /// The claimed evaluation at `ood_point` for each column in the top layer of each instance.
    pub claims_to_verify_by_instance: Vec<Vec<SecureField>>,
    /// The number of variables that interpolate the top layer of each instance.
