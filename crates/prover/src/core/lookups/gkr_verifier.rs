//! Batch GKR protocol implementation designed to verify lookup arguments.
use thiserror::Error;
...
pub fn partially_verify_batch(
    ...
    for (layer, sumcheck_proof) in sumcheck_proofs.iter().enumerate() {
        ...
        for &instance in &sumcheck_instances {
            ...
            let gate_output = gate_by_instance[instance].eval(mask)?;
            let folded_output = random_linear_combination(&gate_output, instance_lambda);
