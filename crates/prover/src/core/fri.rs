...
pub struct FriProver<'columns, B: FriOps + MerkleOps<MC::H>, MC: MerkleChannel> {
    config: FriConfig,
    first_layer: FriFirstLayerProver<'columns, B, MC::H>,
    inner_layers: Vec<FriInnerLayerProver<B, MC::H>>,
    last_layer_poly: LinePoly,
}

impl<'columns, B: FriOps + MerkleOps<MC::H>, MC: MerkleChannel> FriProver<'columns, B, MC> {
    ...
    pub fn commit(
        channel: &mut MC::C,
        config: FriConfig,
        columns: &'columns [SecureEvaluation<B, BitReversedOrder>],
    ...
    fn commit_first_layer(
        channel: &mut MC::C,
        columns: &'columns [SecureEvaluation<B, BitReversedOrder>],
    ) -> FriFirstLayerProver<'columns, B, MC::H> {
    ...
    fn commit_inner_layers(
        ...
        ///
        /// [`CircleEvaluation`]: crate::core::poly::circle::CircleEvaluation
        fn folded_len(v: &SecureEvaluation<impl PolyOps, BitReversedOrder>) -> usize {
            v.len() >> CIRCLE_TO_LINE_FOLD_STEP
        }

        let circle_poly_folding_alpha = channel.draw_felt();
        let first_inner_layer_domain =
            LineDomain::new(Coset::half_odds(folded_len(&columns[0]).ilog2()));
        ...
        while layer_evaluation.len() > config.last_layer_domain_size() {
            // Check for circle polynomials in the first layer should be combined in this layer.
    ...
    /// Generates and returns a FRI proof with the query positions for the committed columns.
    ///
    ...
    pub fn decommit(self, channel: &mut MC::C) -> (FriProof<MC::H>, BTreeMap<u32, Vec<usize>>) {
        ...
        let query_positions_per_log_size =
            get_query_positions_per_log_size(&queries, column_log_sizes);
        let proof = self.decommit_on_queries(&queries);
        (proof, query_positions_per_log_size)
    }
...
impl<MC: MerkleChannel> FriVerifier<MC> {
    ...
    /// Verifies the first layer decommitments.
    ///
    /// Returns the queries and first layer folded column evaluations needed for verifying
    /// the inner FRI layers.
    ...
    pub fn query_positions_per_column_log_size(
        &mut self,
        channel: &mut MC::C,
...
pub enum FriVerificationError {
    ...
    #[error("queries do not resolve to their commitment in inner layer {layer}")]
    InnerLayerCommitmentInvalid {
        layer: usize,
        error: MerkleVerificationError,
    },
    #[error("evaluations are invalid in inner layer {layer}")]
...
impl<H: MerkleHasher> FriFirstLayerVerifier<H> {
    ...
    fn verify_and_fold(
        ...
        let mut missing_evals = self.proof.missing_evals.iter().copied();
            ...
            zip_eq(&self.column_commitment_domains, evals_at_queries_by_column)
        {
            ...
            let (column_decommitment_positions, sparse_evaluation) =
                ...
                .map_err(|InsufficientMissingEvalsError| {
...
impl<H: MerkleHasher> FriInnerLayerVerifier<H> {
    ...
    fn verify_and_fold(
        ...
        let (decommitment_positions, sparse_evaluation) = compute_openings_and_rebuild_evals(
            &queries.positions,
            &evals_at_queries,
            &mut missing_evals,
            FOLD_STEP,
...
impl<'a, B: FriOps + MerkleOps<H>, H: MerkleHasher> FriFirstLayerProver<'a, B, H> {
    ...
    /// Generates a decommitment at the specified query positions.
    fn decommit(self, queries: &Queries) -> FriLayerProof<H> {
        let max_column_log_size = *self.column_log_sizes().iter().max().unwrap();
        assert_eq!(queries.log_domain_size, max_column_log_size);

        let mut missing_evals = Vec::new();
        let mut decommit_positions = BTreeMap::new();

        for column in self.columns {
            let column_log_size = column.domain.log_size();
            let column_queries = queries.fold(queries.log_domain_size - column_log_size);

            let (column_decommit_positions, column_missing_evals) =
                compute_openings_and_missing_evals(
                    column,
                    &column_queries.positions,
                    CIRCLE_TO_LINE_FOLD_STEP,
                );

            decommit_positions.insert(column_log_size, column_decommit_positions);
            missing_evals.extend(column_missing_evals);
        }
...
impl<B: FriOps + MerkleOps<H>, H: MerkleHasher> FriInnerLayerProver<B, H> {
    ...
    fn decommit(self, queries: &Queries) -> FriLayerProof<H> {
        let (decommit_positions, missing_evals) =
            compute_openings_and_missing_evals(&self.evaluation.values, queries, FOLD_STEP);

        let layer_log_size = self.evaluation.domain().log_size();
        let (_evals, decommitment) = self.merkle_tree.decommit(
            &BTreeMap::from_iter([(layer_log_size, decommit_positions)]),
...
/// Returns a column's merkle tree opening positions and missing evals the verifier can't compute
/// but requires for decommitment and folding.
///
/// Output of the form: `(opening_positions, missing_evals)`
fn compute_openings_and_missing_evals(
    ...
    for query_group in query_positions.group_by(|a, b| a >> fold_step == b >> fold_step) {
        let opening_start = (query_group[0] >> fold_step) << fold_step;
        let mut query_group = query_group.iter().peekable();
        let opening_positions = opening_start..opening_start + (1 << fold_step);
        ...
        for position in opening_positions {
            ...
            if query_group.next_if_eq(&&position).is_some() {
                continue;
            }

            let eval = column.at(position);
            missing_evals.push(eval);
        }
    }

    (decommit_positions, missing_evals)
}

/// Returns a column's merkle tree opening positions and re-builds the evaluations needed by
/// the verifier for folding and decommitment.
///
/// # Panics
///
/// Panics if the number of queries doesn't match the number of query evals.
fn compute_openings_and_rebuild_evals(
    queries: &[usize],
    query_evals: &[QM31],
    mut missing_evals: impl Iterator<Item = QM31>,
    fold_step: u32,
    column_log_size: u32,
) -> Result<(Vec<usize>, SparseEvaluation), InsufficientMissingEvalsError> {
    ...
    for subset_queries in queries.group_by(|a, b| a >> fold_step == b >> fold_step) {
        ...
        let subset_opening_positions = subset_start..subset_start + (1 << fold_step);
        opening_positions.extend(subset_opening_positions.clone());

        let mut subset_queries_iter = subset_queries.iter().copied().peekable();

        let subset_eval = subset_opening_positions
            .map(|position| match subset_queries_iter.next_if_eq(&position) {
                Some(_) => Ok(query_evals.next().unwrap()),
                None => missing_evals.next().ok_or(InsufficientMissingEvalsError),
            })
            .collect::<Result<_, _>>()?;
    ...
    Ok((opening_positions, sparse_evaluation))
}

#[derive(Debug)]
struct InsufficientMissingEvalsError;
...
impl SparseEvaluation {
    /// # Panics
    ///
    /// Panics if a subset size doesn't equal the folding factor or there arent the same number of
    /// domain indexes as subsets.
    // TODO(andrew): Find better name than `domain_initial_index_by_subset`.
...
mod tests {
    ...
    fn mixed_degree_proof_with_queries_sampled_from_channel_passes_verification(
        ...
        let prover = FriProver::commit(&mut test_channel(), config, &columns, &twiddles);
        let (proof, prover_query_positions_per_log_size) = prover.decommit(&mut test_channel());
        let query_evals_per_column = columns.map(|eval| {
            let query_positions = &prover_query_positions_per_log_size[&eval.domain.log_size()];
        ...
        let verifier_query_positions_per_log_size =
            verifier.query_positions_per_column_log_size(&mut test_channel());

        assert_eq!(
            prover_query_positions_per_log_size,
            verifier_query_positions_per_log_size
        );
        verifier.decommit(query_evals_per_column.to_vec())
    }
    ...
    fn proof_with_invalid_inner_layer_evaluation_fails_verification() {
        ...
        proof.inner_layers[1].missing_evals.pop();
        let verifier = FriVerifier::commit(&mut test_channel(), config, proof, bound).unwrap();
    ...
    fn proof_with_invalid_inner_layer_decommitment_fails_verification() {
        ...
        proof.inner_layers[1].missing_evals[0] += BaseField::one();
        let verifier = FriVerifier::commit(&mut test_channel(), config, proof, bound).unwrap();
    ...
    fn query_polynomial_at_positions(
        ...
        query_positions.iter().map(|i| polynomial.at(*i)).collect()
    }
}
