...
impl<'a, B: FriOps + MerkleOps<H>, H: MerkleHasher> FriFirstLayerProver<'a, B, H> {
    ...
    fn decommit(self, queries: &Queries) -> FriLayerProof<H> {
        ...
        for column in self.columns {
            ...
            let (column_decommitment_positions, column_missing_evals) =
                compute_decommitment_positions_and_missing_evals(
                    column,
                    &column_queries.positions,
                    CIRCLE_TO_LINE_FOLD_STEP,
                );
...
impl<B: FriOps + MerkleOps<H>, H: MerkleHasher> FriInnerLayerProver<B, H> {
    ...
    fn decommit(self, queries: &Queries) -> FriLayerProof<H> {
        let (decommitment_positions, missing_evals) =
            compute_decommitment_positions_and_missing_evals(
...
/// compute but requires for decommitment and folding.
fn compute_decommitment_positions_and_missing_evals(
    ...
    (decommitment_positions, missing_evals)
}
