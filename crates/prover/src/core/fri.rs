...
impl<H: MerkleHasher> FriVerifier<H> {
    ...
    fn decommit_inner_layers(
        ...
        for layer in self.inner_layers.iter() {
            if column_bounds
                ...
                .is_some()
            {
                // We accumulate before folding, but the previous layers still need to get a
                // factor of alpha^2 for every column combined, even in mixed degree.
                // log base is circle_poly_alpha_sq.
                let mut log_previous_layer_combination_factor = 1;
                    ...
                    .is_some()
                {
                    ...
                    log_previous_layer_combination_factor += 1;
                }
                ...
                let prev_layer_combination_factor =
                    circle_poly_alpha_sq.pow(log_previous_layer_combination_factor);
    ...
    fn decompose(layer_queries: &Queries, evals: &mut SparseCircleEvaluation, lambda: SecureField) {
