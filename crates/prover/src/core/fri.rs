...
pub trait FriOps: FieldOps<BaseField> + PolyOps + Sized + FieldOps<SecureField> {
    ...
    /// Decomposes a polynomial of Degree N/2 in 'x', into a polynomial inside the fft-space and the remainder term.
    /// Based on lemma #12 from the CircleStark paper: f = g + alternating(lambda), where lambda is the
    /// cosset diff of eval, and g is the polynomial inside the fft-space.
