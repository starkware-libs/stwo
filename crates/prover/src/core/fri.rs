    ...
    fn fold_line(self, fold_alpha: SecureField, source_domain: LineDomain) -> Vec<SecureField> {
        zip(self.subset_evals, self.subset_domain_initial_indexes)
            .map(|(eval, domain_initial_index)| {
                ...
                let (_, mut folded_values) = fold_line(eval.into_iter(), fold_domain, fold_alpha);
                folded_values.next().unwrap()
            })
            .collect()
    }
    ...
    fn fold_circle(self, fold_alpha: SecureField, source_domain: CircleDomain) -> Vec<SecureField> {
        zip(self.subset_evals, self.subset_domain_initial_indexes)
            .map(|(eval, domain_initial_index)| {
                ...
                let domain = LineDomain::new(fold_domain.half_coset);
                let mut buffer = vec![SecureField::zero(); domain.size()];
...
pub fn fold_line(
    eval: impl ExactSizeIterator<Item = SecureField>,
    domain: LineDomain,
    alpha: SecureField,
) -> (LineDomain, impl Iterator<Item = SecureField>) {
    assert!(eval.len() >= 2, "Evaluation too small");

    let folded_values =
        eval.into_iter()
            .array_chunks()
            .enumerate()
            .map(move |(i, [f_x, f_neg_x])| {
...
pub fn fold_circle_into_line(
    ...
    assert_eq!(eval.len() >> CIRCLE_TO_LINE_FOLD_STEP, dst.len());
    ...
    eval.iter()
        ...
        .for_each(|(i, [&f_p, &f_neg_p])| {
            // TODO(andrew): Inefficient. Update when domain twiddles get stored in a buffer.
            let p = eval_domain.at(bit_reverse_index(
...
mod tests {
    ...
    fn valid_proof_passes_verification() -> Result<(), FriVerificationError> {
        ...
        let proof = prover.decommit_given_queries(&queries);
    ...
    fn proof_with_invalid_inner_layer_evaluation_fails_verification() {
        ...
        let prover = FriProver::commit(&mut test_channel(), config, columns, &twiddles);
        let bound = vec![CirclePolyDegreeBound::new(LOG_DEGREE)];
        let mut proof = prover.decommit_given_queries(&queries);
