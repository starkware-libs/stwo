...
pub trait FieldExpOps: Mul<Output = Self> + MulAssign + Sized + One + Clone {
    ...
    fn batch_inverse(column: &[Self], dst: &mut [Self]) {
        ...
        let mut cum_prod: [Self; WIDTH] = array::from_fn(|_| Self::one());
        zip(&mut dst[..WIDTH], cum_prod.clone()).for_each(|(dst, v)| *dst = v);
        for i in 0..n {
        ...
        let mut tail_inverses: [Self; WIDTH] = array::from_fn(|_| Self::one());
        ...
        zip(&mut dst[0..WIDTH], tail_inverses).for_each(|(dst, v)| *dst = v);
    }
}
