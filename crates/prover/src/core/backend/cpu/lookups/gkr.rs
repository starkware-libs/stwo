...
impl GkrOps for CpuBackend {
    ...
    fn sum_as_poly_in_first_variable(
        ...
        // 1. `sum_x eq(({0}^|r|, 0, x), y) * F(t, x)`
        // 2. `sum_x eq((r,       t, x), y) * F(t, x)`
        {
    ...
    SecureField: ExtensionOf<F> + Field,
{
    for i in 0..n_terms {
        ...
        let inp_numer_at_r0i0 = numerators[i * 2];
        let inp_denom_at_r0i0 = denominators[i * 2];
        let inp_numer_at_r0i1 = numerators[i * 2 + 1];
        let inp_denom_at_r0i1 = denominators[i * 2 + 1];
        let inp_numer_at_r1i0 = numerators[(n_terms + i) * 2];
        let inp_denom_at_r1i0 = denominators[(n_terms + i) * 2];
        let inp_numer_at_r1i1 = numerators[(n_terms + i) * 2 + 1];
        let inp_denom_at_r1i1 = denominators[(n_terms + i) * 2 + 1];
