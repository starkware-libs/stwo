...
pub fn column_line_coeffs(sample_batches: &[ColumnSampleBatch]) -> Vec<Vec<(CM31, CM31)>> {
    sample_batches
        .iter()
        .map(|sample_batch| {
            sample_batch
                ...
                .map(|(_, sampled_value)| {
                    ...
                    complex_conjugate_line_coeffs(&sample)
                })
                .collect()
        })
        .collect()
...
pub struct QuotientConstants<B: Backend> {
    ...
    pub line_coeffs: Vec<Vec<(CM31, CM31)>>,
