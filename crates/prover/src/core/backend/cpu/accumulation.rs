...
impl AccumulationOps for CpuBackend {
    fn accumulate(column: &mut SecureColumn<Self>, other: &SecureColumn<Self>) {
        ...
        let len = column.len();
        for i in 0..len { //TODO: just for the sake of correctness check - perf optimisation can be done without convertion data
            let ci = column.at(i);
            let oi = other.at(i);
        ...
        a.iter().enumerate().for_each(|(i, &item)| column.set(i, SecureField::from_m31_array(unsafe { std::mem::transmute(item) })));
    }
}
