...
fn accumulate_quotients_on_subdomain(
    ...
    let mut values = unsafe { SecureColumnByCoords::<SimdBackend>::uninit(subdomain.size()) };
    ...
    // Extend the evaluation to the full domain.
    let extended_eval = unsafe { SecureColumnByCoords::<SimdBackend>::uninit(domain.size()) };
...
fn denominator_inverses(
    ...
    let mut flat_denominator_inverses = unsafe { CM31Column::uninit(flat_denominators.len()) };
