...
fn accumulate_quotients_on_subdomain(
    ...
    iter.for_each(|(quad_row, (points, mut dst))| {
        ...
        unsafe {
            dst.set_packed(0, row_accumulator[0]);
            dst.set_packed(1, row_accumulator[1]);
            dst.set_packed(2, row_accumulator[2]);
            dst.set_packed(3, row_accumulator[3]);
        }
    });
    span.exit();
