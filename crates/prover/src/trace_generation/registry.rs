...
mod tests {
    ...
    impl TraceGenerator<CpuBackend> for ComponentACpuTraceGenerator {
        ...
        type Inputs = ComponentACpuInput;
    ...
    impl TraceGenerator<SimdBackend> for ComponentASimdTraceGenerator {
        type Component = ComponentA;
        type Inputs = ComponentASimdInput;
    ...
    fn test_component_registry() {
        ...
        let component_a_cpu_generator_id = "cpu";
        let component_a_simd_generator_id = "simd";

        let component_a_cpu_trace_generator = ComponentACpuTraceGenerator { inputs: vec![] };
        let component_a_simd_trace_generator = ComponentASimdTraceGenerator { inputs: vec![] };
        registry.register(
            component_a_cpu_generator_id,
            component_a_cpu_trace_generator,
        );
        registry.register(
            component_a_simd_generator_id,
            component_a_simd_trace_generator,
        );
        ...
        registry
            .get_generator_mut::<ComponentACpuTraceGenerator>(component_a_cpu_generator_id)
            .add_inputs(&cpu_inputs);
        registry
            .get_generator_mut::<ComponentASimdTraceGenerator>(component_a_simd_generator_id)
