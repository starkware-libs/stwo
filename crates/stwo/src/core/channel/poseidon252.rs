...
impl Poseidon252Channel {
    ...
    fn draw_secure_felt252(&mut self) -> FieldElement252 {
        // The reason for using `poseidon_permute_comp` directly, with `FieldElement252::THREE`,
        // is for domain separation between draw and mix. In all the mix functions, the constant
        // used is `FieldElement252::TWO`.
        let mut state = [
            self.digest,
