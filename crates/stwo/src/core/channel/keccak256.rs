...
mod tests {
    ...
    /// Solidity-equivalence KAT: `mix_u64(value)` should equal
    /// `keccak256(abi.encodePacked(prev_digest, uint64(value)))` where
    /// `prev_digest` is the all-zero default initial digest.
    #[test]
    pub fn test_mix_u64_solidity_kat() {
        let mut channel = Keccak256Channel::default();
    ...
    /// `mix_u64` and `mix_u32s(&[high, low])` (BE word order) must agree, since
    /// `u64::to_be_bytes` is `[high_u32_be, low_u32_be]` concatenated.
    #[test]
    pub fn test_mix_u64_matches_mix_u32s_be() {
        let mut channel = Keccak256Channel::default();
        channel.mix_u64(0x1111222233334444);
        let digest_64 = channel.digest;

        let mut channel = Keccak256Channel::default();
        channel.mix_u32s(&[0x11112222, 0x33334444]);

        assert_eq!(digest_64, channel.digest);
    }

    /// Solidity-equivalence KAT for `mix_u32s` against
    /// `keccak256(abi.encodePacked(prev_digest, uint32(...) ...))`.
    #[test]
    pub fn test_mix_u32s_solidity_kat() {
        let mut channel = Keccak256Channel::default();
