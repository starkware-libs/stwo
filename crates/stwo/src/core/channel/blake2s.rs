...
use crate::core::vcs::blake2_hash::{Blake2sHash, Blake2sHasherImpl};

pub const BLAKE_BYTES_PER_HASH: usize = 32;
pub const FELTS_PER_HASH: usize = 8;

pub type Blake2sChannel = Blake2sChannelImpl<false>;
/// Same as [Blake2sChannel], expect that the hash output is taken modulo M31::P.
pub type Blake2sM31Channel = Blake2sChannelImpl<true>;

/// A channel that can be used to draw random elements from a [Blake2sHash] digest.
#[derive(Default, Clone, Debug)]
pub struct Blake2sChannelImpl<const IS_M31_OUTPUT: bool> {
    digest: Blake2sHash,
    n_draws: u32,
}

impl<const IS_M31_OUTPUT: bool> Blake2sChannelImpl<IS_M31_OUTPUT> {
...
impl<const IS_M31_OUTPUT: bool> Channel for Blake2sChannelImpl<IS_M31_OUTPUT> {
    ...
    fn mix_felts(&mut self, felts: &[SecureField]) {
        let felts_bytes = felts
            ...
            .collect_vec();
        let mut hasher = Blake2sHasherImpl::<IS_M31_OUTPUT>::new();
    ...
    fn draw_u32s(&mut self) -> Vec<u32> {
        ...
        self.n_draws += 1;
        Blake2sHasherImpl::<IS_M31_OUTPUT>::hash(&hash_input)
            .0
            .chunks_exact(4)
    ...
    fn verify_pow_nonce(&self, n_bits: u32, nonce: u64) -> bool {
        ...
        let mut hasher = Blake2sHasherImpl::<IS_M31_OUTPUT>::default();
