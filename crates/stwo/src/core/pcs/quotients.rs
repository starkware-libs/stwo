...
impl ColumnSampleBatch {
    ...
    pub fn new_vec_with_periodicity_samples(
        ...
        let mut grouped_samples = IndexMap::default();
        let max_domain_generator = CanonicCoset::new(max_log_size).step();
        let periodicity = max_domain_generator.repeated_double(log_size);
        for (column_index, samples) in samples.iter().enumerate() {
            for sample in samples.iter() {
                grouped_samples
                    .entry(sample.point)
            ...
            if samples.len() >= 2 {
                ...
                grouped_samples
                    .entry(last_sample.point.add(periodicity.into_ef()))
                    .or_insert_with(Vec::new)
                    .push((column_index, last_sample.value));
            }
        }
        grouped_samples
            .into_iter()
