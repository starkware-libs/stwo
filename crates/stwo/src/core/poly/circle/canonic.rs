...
pub struct CanonicCosetLogSizeError {
    pub log_size: u32,
}
...
impl CanonicCoset {
    pub fn try_new(log_size: u32) -> Result<Self, CanonicCosetLogSizeError> {
        if log_size == 0 || log_size > MAX_CANONICAL_COSET_LOG_SIZE {
            return Err(CanonicCosetLogSizeError { log_size });
        }
        Ok(Self {
