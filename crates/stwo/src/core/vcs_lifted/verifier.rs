...
impl<H: MerkleHasherLifted> MerkleVerifierLifted<H> {
    ...
    pub fn verify(
        ...
        let queried_values_sorted_by_col = queried_values
            .into_iter()
        ...
        for (idx, pos) in query_positions.iter().enumerate() {
            let values = queried_values_sorted_by_col
                .iter()
                .map(|col_vals| col_vals[idx])
                .collect_vec();
            let maybe_old_values = positions_and_values.get(pos);
            match maybe_old_values {
                // If we encounter `pos` for the first time, we insert the queried values.
                None => {
                    positions_and_values.insert(*pos, values);
                }
        ...
        for (pos, values) in positions_and_values
            .into_iter()
