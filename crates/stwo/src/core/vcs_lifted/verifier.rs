...
pub struct MerkleVerifierLifted<H: MerkleHasherLifted> {
    ...
    /// When the Merkle verifier is part of a commitment scheme involving multiple trees, this
    /// integer is the max log size of a committed column across all trees. When the Merkle
    /// verifier is used as a standalone (e.g. in FRI verification), `global_domain_log_size` is
    /// equal to the maximum in `column_log_sizes`.
    pub global_domain_log_size: u32,
}

impl<H: MerkleHasherLifted> MerkleVerifierLifted<H> {
    pub fn new(root: H::Hash, column_log_sizes: Vec<u32>, global_domain_log_size: u32) -> Self {
    ...
    /// # Arguments
    ///
    /// * `query_positions` - Indices of the query positions (in range `[0,
    ///   2^self.global_domain_log_size)`), in increasing order. Note that both the ordering and the
    ///   value bounds are not checked in this function.
    ...
    pub fn verify(
        ...
        let log_ratio = self.global_domain_log_size - max_log_size;
