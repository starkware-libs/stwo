...
// TODO(Leo): the fields column_log_sizes and n_colums_per_log_size
// contain more information than needed for implementing a merkle verifier
// (knowing the max and length of column_log_sizes is enough). However,
// this info is needed by the pcs and storing it here makes integration easier.
// Consider refactoring the API.
pub struct MerkleVerifierLifted<H: MerkleHasherLifted> {
    /// The commitment value.
    pub root: H::Hash,
    /// A vector containing the log sizes of the columns that
    /// were committed, in the order they were sent to the MerkleProver
    /// (i.e. before sorting).
    pub column_log_sizes: Vec<u32>,
    /// A dictionary mapping an integer n to the number of columns
    /// of log size n.
    pub n_columns_per_log_size: BTreeMap<u32, usize>,
    /// When the Merkle verifier is part of a commitment scheme involving multiple
    /// trees, this integer is the max log size of a committed column across
    /// all trees. When the Merkle verifier is used as a standalone
    /// (e.g. in FRI verification), it is equal to the max in column_log_sizes.
    pub max_log_size_across_trees: u32,
}

impl<H: MerkleHasherLifted> MerkleVerifierLifted<H> {
    pub fn new(root: H::Hash, column_log_sizes: Vec<u32>, max_log_size_across_trees: u32) -> Self {
    ...
    /// # Arguments
    ///
    /// * `queries_position` - Indices of the query positions (in range `[0,
    ///   2^self.max_log_size_across_trees)`), in increasing order. Note that both the ordering and
    ///   the value bounds are not checked in this function.
    ...
    pub fn verify(
        ...
        let log_ratio = self.max_log_size_across_trees - max_log_size;
        let mut prev_layer_hashes: Vec<(usize, H::Hash)> = queries_position
            ...
            .map(|(idx, column_values)| {
                ...
                ((idx >> (log_ratio + 1) << 1) + (idx & 1), hasher.finalize())
            })
            .collect();
