...
/// Extension trait that brings back `checked_as_chunks` / `checked_as_chunks_mut` on slices.
///
/// Rust 1.94 removed the unstable `slice::checked_as_chunks` method. This trait provides equivalent
/// methods via `as_chunks`, asserting that no remainder is left over.
pub trait SliceExt {
    type Item;
