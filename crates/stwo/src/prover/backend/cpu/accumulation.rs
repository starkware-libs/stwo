...
impl AccumulationOps for CpuBackend {
    ...
    fn lift_and_accumulate(
        ...
        let start_log_size = 1;
        assert!(
            cols[0].len() >= 1 << start_log_size,
            "A column must be of length at least 2."
        );
        let mut curr = SecureColumnByCoords::zeros(1 << start_log_size);
