...
pub trait Column<T>: Clone + Debug + FromIterator<T> + Send + Sync {
    ...
    /// blocking default-stream read, cutting the per-hop latency. CPU/SIMD keep the default, so their
    /// behavior is unchanged. The only observable change is the transfer path, never the bytes.
