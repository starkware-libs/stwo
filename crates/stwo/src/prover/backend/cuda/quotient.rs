...
// quotient scheme byte-identically to SimdBackend (crates/stwo/src/prover/backend/simd/quotients.rs).
//
...
//   SUBDOMAIN (giving the quotient on the subdomain, mirroring simd quotients.rs L116-179), then the
//   on-device lift: for each of the 4 secure coords, interpolate the subdomain eval -> coeffs and
//   evaluate on the full domain (simd quotients.rs L188-197).
//
// SAFETY OF THE PORT:
...
// * The kernel's `domain_at_index(half_coset.initial_index, half_coset.step_size, bit_reverse(row))`
//   convention matches simd's `CircleDomainBitRevIterator` (== `domain.at(bit_reverse_index(row))`),
//   the same convention validated byte-identical-to-CPU by the barycentric path/tests.
...
fn accumulate_numerators_simd_delegated(
    ...
    SimdBackend::accumulate_numerators(&simd_refs, sample_batches, &mut simd_acc, log_blowup_factor);
    for a in simd_acc {
...
impl QuotientOps for CudaBackend {
    fn accumulate_numerators(
        ...
        // Build a device array of device column pointers once (the accumulate kernel dereferences
        // `columns` on-device). The columns are the committed, device-resident CircleEvaluations.
        ...
        // simd quotients.rs L67-77: per sample batch, accumulate the numerators on the subdomain
        // (numerator term = c * col[row] - b), and accumulate the `a` line coeffs separately.
        ...
        for (batch, coeffs) in sample_batches.iter().zip(quotient_constants.line_coeffs) {
            // Per-term flattened (b, c, column_index) for this batch, matching the kernel's
            // `acc += c_j * columns[column_indices[j]][row] - b_j` (cuda/quotients.cu L186-207).
            // simd's `accumulate_numerators_on_subdomain` uses exactly `(_, b, c)` per term
            // (simd quotients.rs L235-243).
            ...
            // Result secure column on the subdomain (4 device coords: a.a, a.b, b.a, b.b).
            let result = unsafe {
                CudaSecureColumn::new_with_size(subdomain_size)
            };
            unsafe {
            ...
            // simd quotients.rs L71: first_linear_term_acc = sum of the `a` line coeffs.
