...
mod tests {
    ...
    fn test_finalize_sequence_20_to_24() {
        ...
        let gpu_evals: [Vec<BaseField>; 4] = gpu_polys
            .map(|p| CudaBackend::evaluate(&p, large_domain, &gpu_large_twiddles).values.to_cpu());
        ...
        let mut gpu_secure_col = SecureColumnByCoords { columns: gpu_values_large };
        let gpu_eval_secure_col =
            SecureColumnByCoords { columns: gpu_evals.map(BaseFieldVec::from_vec) };
        ...
        for (i, cpu_col) in cpu_accumulated.iter().enumerate() {
    ...
    fn test_poly_extend_evaluate_log19() {
        ...
        let cpu_coeffs: Vec<BaseField> =
            (0..small_size).map(|i| BaseField::from((i * 7 + 13) as u32)).collect();
    ...
    fn test_finalize_sequence_7_to_19() {
        ...
        for i in 0..4 {
            ...
            let mismatches: Vec<_> =
                (0..cpu_coeffs.len()).filter(|&j| cpu_coeffs[j] != gpu_coeffs[j]).collect();
        ...
        let gpu_eval: [Vec<BaseField>; 4] = gpu_prev_poly
            .map(|p| CudaBackend::evaluate(&p, large_domain, &gpu_twiddles).values.to_cpu());

        // Verify evaluation matches
        for i in 0..4 {
            let mismatches: Vec<_> =
                (0..large_size).filter(|&j| cpu_eval[i][j] != gpu_eval[i][j]).collect();
            if !mismatches.is_empty() {
                println!(
        ...
        let mut gpu_values_col = SecureColumnByCoords { columns: gpu_large_values };
        let gpu_eval_col = SecureColumnByCoords { columns: gpu_eval.map(BaseFieldVec::from_vec) };
        ...
        for (i, cpu_col) in cpu_accumulated.iter().enumerate() {
            ...
            let mismatches: Vec<_> =
                (0..large_size).filter(|&j| cpu_col[j] != gpu_acc[j]).collect();
            if !mismatches.is_empty() {
                println!(
        ...
        for i in 0..4 {
            ...
            let mismatches: Vec<_> =
                (0..large_size).filter(|&j| cpu_coeffs[j] != gpu_coeffs[j]).collect();
            if !mismatches.is_empty() {
                println!(
