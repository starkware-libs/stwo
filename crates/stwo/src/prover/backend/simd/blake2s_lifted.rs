    ...
    for SimdBackend
{
    ...
    fn pack_leaves_input(values: &[Col<Self, BaseField>; 4]) -> [Col<Self, BaseField>; 16] {
        pack_leaves_input_simd(values)
    }
}

pub fn pack_leaves_input_simd(
    values: &[Col<SimdBackend, BaseField>; 4],
) -> [Col<SimdBackend, BaseField>; 16] {
    ...
    let mut packed_simd: [Vec<PackedBaseField>; 16] =
    ...
    for row in 0..output_packed_len_floor {
        ...
        for (offset, packed_leaf_column) in packed_row.into_iter().enumerate() {
            for coord in 0..SECURE_EXTENSION_DEGREE {
                packed_simd[coord + offset * PACKED_LEAF_SIZE][row] = packed_leaf_column[coord];
            }
        }
    }
    ...
    if tail_rows > 0 {
        let mut tail_columns: [[BaseField; N_LANES]; 16] =
        ...
        for row in 0..tail_rows {
            ...
            for offset in 0..PACKED_LEAF_SIZE {
                ...
                for coord in 0..SECURE_EXTENSION_DEGREE {
                    tail_columns[coord + offset * PACKED_LEAF_SIZE][row] = coords[coord];
                }
            }
        }
        for column_idx in 0..16 {
