    ...
    for SimdBackend
{
    ...
    fn build_leaves(
        ...
        let mut res = Vec::with_capacity(
            1 << (max_log_size + additional_lifting_ratio + LOG_N_HASHES_PER_SIMD_STATE),
        );
        ...
        let mut trasposed_states = if additional_lifting_ratio == 0 {
            next_layer_states
        } else {
            let mut tmp: Vec<[u32x16; N_FELTS_IN_BLAKE_STATE]> =
                Vec::with_capacity(1 << (max_log_size + additional_lifting_ratio));
            unsafe {
                tmp.set_len(1 << (max_log_size + additional_lifting_ratio));
            }
