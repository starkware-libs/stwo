...
impl MerkleOpsLifted<Keccak256MerkleHasher> for SimdBackend {
    ...
    fn build_leaves(
        ...
        for group_cols in columns.chunk_by(|a, b| a.len() == b.len()) {
            let log_cur_size = group_cols[0].len().ilog2();
            debug_assert!(log_cur_size >= LOG_N_LANES);
            let log_n_sponges = log_cur_size - LOG_N_LANES_KECCAK;
        ...
        // Sanity check: the buffer should be fully populated by the final lifting iteration
        // (either the last iteration of the loop or the `if` above).
        debug_assert_eq!(prev_layer.len(), 1usize << prev_log_n_sponges);
