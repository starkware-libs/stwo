...
impl AccumulationOps for SimdBackend {
    ...
    fn lift_and_accumulate(
        ...
        if cols[0].len() == 0 {
            panic!("Columns should be non-empty.")
        }

        let mut prev = SecureColumnByCoords::zeros(1);
        for mut col in cols.into_iter() {
            ...
            for i in 0..col.len() >> LOG_N_LANES {
                unsafe {
                    ...
                    let res =
                        col.packed_at(i) + PackedSecureField::from_packed_m31s(packed_after_lift);
                    col.set_packed(i, res);
                }
            }
            prev = col;
        }
