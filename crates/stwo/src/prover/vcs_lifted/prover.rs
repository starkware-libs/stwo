...
impl<B: MerkleOpsLifted<H>, H: MerkleHasherLifted> MerkleProverLifted<B, H> {
    ...
    pub fn decommit(
        ...
        for layer_log_size in (0..self.layers.len() - 1).rev() {
            ...
            let mut prev_layer_queries_peekable = prev_layer_queries.into_iter().peekable();

            while let Some(node_index) = prev_layer_queries_peekable.peek().map(|q| q / 2) {
                // If the left child was not computed, add it to the witness.
                if prev_layer_queries_peekable
                    .next_if_eq(&(2 * node_index))
                    .is_none()
                {
                    decommitment
                        .hash_witness
                        .push(prev_layer_hashes.at(2 * node_index));
                }
                    ...
                    .is_none()
                {
                    decommitment
                        .hash_witness
                        .push(prev_layer_hashes.at(2 * node_index + 1));
                }

                curr_layer_queries.push(node_index);
            }
...
mod test {
    ...
    fn test_bit_reverse_lifted_merkle_cpu() {
        ...
        let merkle_prover_0 = MerkleProver::<CpuBackend, Blake2sMerkleHasherCurrent>::commit(vec![
            &lifted_evaluation.values,
            &last_column,
        ]);
        let merkle_prover_1 = MerkleProverLifted::<CpuBackend, Blake2sMerkleHasher>::commit(vec![
            &lifted_evaluation.values,
            &last_column,
        ]);
        let merkle_prover_2 = MerkleProverLifted::<CpuBackend, Blake2sMerkleHasher>::commit(vec![
            &poly.evaluate(domain),
            &last_column,
        ]);

        assert_eq!(merkle_prover_1.root(), merkle_prover_2.root());
        assert_eq!(merkle_prover_0.root(), merkle_prover_1.root());
    }
}
