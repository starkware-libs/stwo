...
impl<B: MerkleOpsLifted<H>, H: MerkleHasherLifted> MerkleProverLifted<B, H> {
    ...
    pub fn decommit(
        ...
        for layer_log_size in (0..self.layers.len() - 1).rev() {
            ...
            for chunk in prev_layer_queries.as_slice().chunk_by(|a, b| a ^ 1 == *b) {
                let first = chunk[0];
                ...
                curr_layer_queries.push(chunk[0] >> 1);
            }
...
mod test {
    ...
    fn test_small_col_sizes() {
        ...
        // Check Merkle commitment on columns all of log_size == 0.
        let cols = [vec![M31::from(0)], vec![M31::from(1)], vec![M31::from(2)]];
        let mixed_degree_merkle_prover =
            MerkleProver::<CpuBackend, Blake2sMerkleHasherCurrent>::commit(cols.iter().collect());
        let lifted_merkle_prover =
