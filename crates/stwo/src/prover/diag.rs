...
#[inline]
// Not `const`: with the `cuda` feature the body calls the CUDA-runtime FFI (non-const). Only the
// no-op non-cuda build is const-able, so `missing_const_for_fn` doesn't hold across feature
// configs.
