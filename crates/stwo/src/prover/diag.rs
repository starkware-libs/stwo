...
//! so the timers are available on the production-fast binary (the numbers reflect production). Emits
//! `[prove_ex] <name> <secs>s` lines. Does NOT change any prover logic or proof output (byte-identity
//! is guarded by the proof fingerprint); each read is ~free when off.
//!
...
/// once-per-prove `prove_ex_timers_on()` read the caller already holds (avoids a re-read per phase).
#[inline]
