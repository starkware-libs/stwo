...
impl<B: ColumnOps<F>, F: ExtensionOf<BaseField>, EvalOrder> CircleEvaluation<B, F, EvalOrder> {
    ...
    ///
    /// The minimal viable domain has `log_size = 2`, which gives a `half_coset` with `log_size = 1`
    /// (non-degenerate). This is the smallest domain that can be used for interpolation.
    ///
    ...
    ///
    /// The domain must have `log_size >= 2` to ensure a non-degenerate `half_coset`.
    pub fn zero_padding_with_log_size(log_size: u32) -> Self {
        assert!(
            log_size >= MIN_CIRCLE_DOMAIN_LOG_SIZE,
            "Domain log_size must be at least 2 for a viable domain"
        );
