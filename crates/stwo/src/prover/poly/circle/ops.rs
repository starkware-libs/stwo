...
pub trait PolyOps: ColumnOps<BaseField> + ColumnOps<SecureField> + Sized {
    ...
    /// (Option A) instead of the per-column `map_cols`/`par_map_cols` closure. `false` by default so
    /// CPU/SIMD keep EXACTLY their pre-existing path (including rayon parallelism across columns).
    /// The Cuda backend sets this `true`: its per-eval work is a tiny kernel + full-device sync + 4
    /// KB D2H, so batching all launches behind one terminal sync + one bulk D2H is the win, and host
    /// parallelism across columns buys nothing there.
    ...
    /// values). CPU/SIMD never reach this (their `USE_BATCHED_OODS` stays `false`); it exists so the
    /// generic call site type-checks for every backend.
