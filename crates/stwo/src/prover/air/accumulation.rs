...
pub trait AccumulationOps: ColumnOps<BaseField> + Sized {
    ...
    /// Receives a possibly empty vector of columns, sorted increasingly
    /// by column length, and returns a column which is the coordinate-wise
    /// sum of the lifts of the columns (see also
    /// [`crate::prover::backend::simd::blake2s_lifted::to_lifted_simd`]). The size of the
    /// output column is equal to the size of the largest column (e.g. the size the last one).
    ///
