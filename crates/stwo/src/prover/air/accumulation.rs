...
mod tests {
    ...
    fn test_non_canonical_domain_accumulation() {
        ...
        // Both should produce the same polynomial (compare all coefficients).
        for (coeffs_sub, coeffs_canon) in poly_sub.0.iter().zip(poly_canon.0.iter()) {
            assert_eq!(coeffs_sub.coeffs, coeffs_canon.coeffs);
        }
    }
}
