...
impl From<SecureField> for CudaSecureField {
    fn from(value: SecureField) -> Self {
        Self { a: value.0.0, b: value.0.1, c: value.1.0, d: value.1.1 }
    }
}
...
impl From<CirclePoint<SecureField>> for CirclePointSecureField {
    fn from(value: CirclePoint<SecureField>) -> Self {
        Self { x: CudaSecureField::from(value.x), y: CudaSecureField::from(value.y) }
    }
}
...
unsafe extern "C" {
    ...
    // One-shot pool defrag: sync + cudaMemPoolTrimTo(0), releasing ALL cached already-freed
    // segments to the OS, so the next shard starts from a clean pool. Live buffers untouched.
    // See utils.cu. Invoked at a shard boundary by the resident multi-shard path; the decision
    // to call it is made by the downstream caller (via its own env flag) — this decl is just
    // the FFI entry point and reads no env var here.
    ...
    // Option A (batched OODS): one call launches all `n` independent dot-product kernels (no
    // interior sync), does a single terminal device sync + single bulk D2H, then the same
    // per-column CPU reduction the single wrapper does. `evals`/`weights` are arrays of `n`
    // device pointers, `sizes` an array of `n` domain sizes; `results` (len `n`) receives one
    // value per eval. Equivalent to calling `barycentric_eval_at_point_cuda` `n` times; only
    // the schedule changes. The caller MUST keep each `evals[e]` buffer alive until this
    // returns (the terminal sync guarantees all kernels have consumed their inputs by then).
