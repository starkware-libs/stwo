...
#include "gen_jump_opcode_trace.cuh"
#include "gen_memory_address_to_id_trace.cuh"
#include "gen_memory_id_to_big_trace.cuh"

// Constants defined in gen_jump_opcode_trace.cuh:
// JUMP_OPCODE_N_TRACE_COLUMNS = 15
// JUMP_OPCODE_N_INTERACTION_TRACE_COLUMNS = 3 (3 logup columns * 4 M31 = 12 M31 columns)
// JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX = 256

__launch_bounds__(256, 2)
__global__ void generate_jump_opcode_trace_kernel(
    m31 **traces,
    ...
    if (row < trace_size) {
        // Input columns
        m31 input_pc_col0 = jump_opcode_inputs[0][row];
        traces[0][row] = input_pc_col0;
        m31 input_ap_col1 = jump_opcode_inputs[1][row];
        traces[1][row] = input_ap_col1;
        m31 input_fp_col2 = jump_opcode_inputs[2][row];
        ...
        // [pc, 32767, 32767, offset2, ((24 + (op1_base_fp * 64)) + (op1_base_ap * 128)), (2 + (ap_update_add_1 * 32)), 0]
        m31 flags0 = add(add(M31_24, mul(op1_base_fp_col4, M31_64)), mul(op1_base_ap_col5, M31_128));
        m31 flags1 = add(M31_2, mul(ap_update_add_1_col6, M31_32));
        ...
        // Write limbs 0-3 (columns 9-12)
        m31 next_pc_limb_0_col9 = next_pc_value[0];
        traces[9][row] = next_pc_limb_0_col9;
        m31 next_pc_limb_1_col10 = next_pc_value[1];
        traces[10][row] = next_pc_limb_1_col10;
        m31 next_pc_limb_2_col11 = next_pc_value[2];
        traces[11][row] = next_pc_limb_2_col11;
        m31 next_pc_limb_3_col12 = next_pc_value[3];
        traces[12][row] = next_pc_limb_3_col12;

        // partial_limb_msb = (limb3 & 2) >> 1
        uint16_t partial_limb_msb_tmp = (((uint16_t)next_pc_limb_3_col12) & UInt16_2) >> UInt16_1;
        m31 partial_limb_msb_col13 = m31{partial_limb_msb_tmp};
        traces[13][row] = partial_limb_msb_col13;

        sub_component_inputss_memory_id_to_big[0][row] = next_pc_id_col8;

        // memory_id_to_big_0 lookup data (only first 5 limbs used, rest are zeros)
        lookup_memory_id_to_big_0[0][row] = next_pc_id_col8;
        lookup_memory_id_to_big_0[1][row] = next_pc_limb_0_col9;
        lookup_memory_id_to_big_0[2][row] = next_pc_limb_1_col10;
        lookup_memory_id_to_big_0[3][row] = next_pc_limb_2_col11;
        lookup_memory_id_to_big_0[4][row] = next_pc_limb_3_col12;
        for (int i = 5; i < 29; i++) {
        ...
        // next_pc = limb0 + limb1 * 512 + limb2 * 262144 + limb3 * 134217728
        m31 next_pc = add(add(add(
            next_pc_limb_0_col9,
        ...
        // Enabler column (col 14)
        traces[14][row] = (row < n_rows) ? M31_1 : M31_0;
    }
}

void generate_jump_opcode_traces(
    unsigned **traces,
    ...
    m31 **device_traces = clone_to_device<m31*>((m31 **)traces, JUMP_OPCODE_N_TRACE_COLUMNS);
    ...
    m31 **device_jump_opcode_input = clone_to_device<m31 *>((m31 **)jump_opcode_input, 3);
    unsigned **device_memory_id_to_big_transposed_big_values = clone_to_device<m31 *>((m31 **)memory_id_to_big_transposed_big_values, 8);


    timer global_timer;
    global_timer.start("generate jump_opcode base trace");

    unsigned trace_size = 1 << log_size;
    int block_dim = trace_size < JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX ? trace_size : JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX;
    int num_blocks = block_dim < JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;

    generate_jump_opcode_trace_kernel<<<num_blocks, block_dim>>>(
        device_traces,
    ...
    cuda_free_memory(device_jump_opcode_input);
...
__launch_bounds__(256, 2)
__global__ void generate_jump_opcode_interaction_trace_col_gen_kernel_round0(
...
template <int N, int M>
__launch_bounds__(256, 2)
__global__ void generate_jump_opcode_interaction_trace_col_gen_kernel_second2last(
    LookupElementsBasic<N> *lookup_elements_n,
    LookupElementsBasic<M> *lookup_elements_m,
    m31 **lookup_state_0,
    m31 **lookup_state_1,
...
template <int N>
__launch_bounds__(256, 2)
__global__ void generate_jump_opcode_interaction_trace_col_gen_kernel_last(
    LookupElementsBasic<N> *lookup_elements_n,
    m31 **lookup_state_0,
    unsigned trace_size,
    qm31 *denom_ptr,
...
__global__ void generate_jump_opcode_interaction_trace_finalize_col_kernel(
    unsigned rep_index,
    unsigned trace_size,
    qm31 *denom_inv_ptr,
    m31 *numerator0,
...
__global__ void generate_jump_opcode_interaction_trace_cumsum_shift(
    unsigned last_index,
    unsigned trace_size,
    m31 **interaction_traces,
    m31 *coordinate_sums
...
__global__ void generate_jump_opcode_interaction_trace_coord_prefix_sum(
    m31 *coordinate_sums,
    unsigned last_index,
    unsigned trace_size,
    m31 **interaction_traces
...
void generate_jump_opcode_interaction_traces(
    ...
    m31 **device_interaction_traces = clone_to_device<m31*>((m31 **)interaction_traces, 4 * JUMP_OPCODE_N_INTERACTION_TRACE_COLUMNS);
    ...
    timer global_timer;
    global_timer.start("generate jump_opcode interaction trace");

    int block_dim = 0;
    int num_blocks = 0;

    block_dim = trace_size < JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX ? trace_size : JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX;
    num_blocks = block_dim < JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;

    // #0 Interaction trace For verify_instruction_0 & memory_address_to_id_0
    generate_jump_opcode_interaction_trace_col_gen_kernel_round0<7, 2><<<num_blocks, block_dim>>>(
    ...
    generate_jump_opcode_interaction_trace_finalize_col_kernel<<<num_blocks, block_dim>>>(
        0,
        trace_size,
        denom_inv,
        device_numerator0,
    ...
    block_dim = trace_size < JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX ? trace_size : JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX;
    num_blocks = block_dim < JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;
    generate_jump_opcode_interaction_trace_col_gen_kernel_second2last<29, 3><<<num_blocks, block_dim>>>(
    ...
    block_dim = trace_size < JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX ? trace_size : JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX;
    num_blocks = block_dim < JUMP_OPCODE_TRACE_GEN_THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;
    generate_jump_opcode_interaction_trace_col_gen_kernel_last<3><<<num_blocks, block_dim>>>(
        device_opcodes_lookup_elements,
        device_lookup_opcodes_1,
        trace_size,
        device_logup_denom,
    ...
    generate_jump_opcode_interaction_trace_cumsum_shift<<<num_blocks, block_dim, shared_size>>>(
        JUMP_OPCODE_N_INTERACTION_TRACE_COLUMNS,
        trace_size,
        device_interaction_traces,
        (m31 *)claimed_sum
    );
    ...
    block_dim = trace_size < THREAD_COUNT_MAX ? trace_size : THREAD_COUNT_MAX;
    num_blocks = block_dim < THREAD_COUNT_MAX ? 1 : (trace_size + block_dim - 1) / block_dim;
    generate_jump_opcode_interaction_trace_coord_prefix_sum<<<num_blocks, block_dim>>>(
        (m31 *)claimed_sum,
        JUMP_OPCODE_N_INTERACTION_TRACE_COLUMNS,
        trace_size,
        device_interaction_traces
    ...
    inclusive_prefix_sum((m31 *)interaction_traces[4 * JUMP_OPCODE_N_INTERACTION_TRACE_COLUMNS - 3], trace_size);
    inclusive_prefix_sum((m31 *)interaction_traces[4 * JUMP_OPCODE_N_INTERACTION_TRACE_COLUMNS - 2], trace_size);
    inclusive_prefix_sum((m31 *)interaction_traces[4 * JUMP_OPCODE_N_INTERACTION_TRACE_COLUMNS - 1], trace_size);

    global_timer.end("generate jump_opcode interaction trace");
