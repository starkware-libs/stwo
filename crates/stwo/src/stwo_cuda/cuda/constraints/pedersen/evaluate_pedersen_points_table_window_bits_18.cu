/*
============================================
PedersenPointsTableWindowBits18 CUDA AIR Evaluator
============================================
...
Functionality:
- Pedersen hash lookup table (window_bits_18 variant)
- Provides precomputed elliptic curve points for Pedersen hash

Data Structure:
- 1 trace column: multiplicity_0 (lookup table usage count)
- 57 preprocessed columns: seq_23, pedersen_points_0..55
- Total: 1 trace column

Constraint Logic:
- No algebraic constraints
- 1 relation lookup (PROVIDE side): uses PEDERSEN_POINTS_TABLE_RELATION_ID
  with 58 values: (relation_id, seq_23, pedersen_points_0..55)
...
#include "evaluate_pedersen_points_table_window_bits_18.cuh"
#include "evaluate_common.cuh"

#define PEDERSEN_POINTS_TABLE_WINDOW_BITS_18_THREAD_COUNT_MAX 256
...
__global__ void evaluate_pedersen_points_table_window_bits_18_pre_kernel(
    qm31 *numerators,
    ...
    PedersenPointsTableWindowBits18_Eval *component_eval,
    qm31 cumsum_shift,
    ...
    // Evaluator for preprocessed trace (trace0): reads seq_23 + pedersen_points_0..55
    ...
    // seq_23: sequence column (table index)
    m31 seq_23 = cuda_evaluator0.next_trace_mask();

    // pedersen_points_0..55: precomputed elliptic curve point coordinates
    ...
    //   [0] = PEDERSEN_POINTS_TABLE_RELATION_ID  (M31(1444721856))
    //   [1] = seq_23
    //   [2..57] = pedersen_points_0..55
    m31 lookup_values[58];
    lookup_values[0] = PEDERSEN_POINTS_TABLE_RELATION_ID;
    lookup_values[1] = seq_23;
    for (int i = 0; i < 56; i++) {
...
// ============================================================================
extern "C"
void evaluate_pedersen_points_table_window_bits_18(
    ...
    PedersenPointsTableWindowBits18_Eval *device_component_eval =
        cuda_malloc<PedersenPointsTableWindowBits18_Eval>(1);
    cuda_mem_copy_host_to_device<PedersenPointsTableWindowBits18_Eval>(
        (PedersenPointsTableWindowBits18_Eval *)eval, device_component_eval, 1);
    ...
    timer global_timer;
    global_timer.start("evaluate_pedersen_points_table_window_bits_18");

    int block_dim = eval_domain_size < PEDERSEN_POINTS_TABLE_WINDOW_BITS_18_THREAD_COUNT_MAX
        ? eval_domain_size
        : PEDERSEN_POINTS_TABLE_WINDOW_BITS_18_THREAD_COUNT_MAX;
    int num_blocks = (eval_domain_size + block_dim - 1) / block_dim;

    if (use_assert_evaluator) {
        evaluate_pedersen_points_table_window_bits_18_pre_kernel<CudaAssertEvaluator><<<
        ...
        evaluate_pedersen_points_table_window_bits_18_pre_kernel<CudaEvaluator><<<
            num_blocks, block_dim, 0, stream>>>(
            numerators,
            device_trace0_evaluations,
            device_trace1_evaluations,
    ...
    ASSERT_CUDA_SUCCESS(cudaStreamSynchronize(stream));
    ASSERT_CUDA_SUCCESS(cudaGetLastError());
    global_timer.end("evaluate_pedersen_points_table_window_bits_18");
